...
TILE_DIR="${BASE}/04_tiles_tif_${PATCH}"
LOG_DIR="${BASE}/logs"
...
echo "=== [${STATE}] Step0: scan tifs ==="
find "$SRC_DIR" -maxdepth 1 -type f \( -name "*.tif" -o -name "*.tiff" \) | sort > "$RAW_LIST"
echo "[INFO] tif count = $(wc -l < "$RAW_LIST")"
if [ "$(wc -l < "$RAW_LIST")" -eq 0 ]; then
  echo "[ERROR] no tif found in $SRC_DIR"; exit 2
fi

echo "=== [${STATE}] Step1: build EPSG map (QA) ==="
: > "$EPSG_MAP"
while read -r f; do
  epsg=$(gdalsrsinfo -o epsg "$f" 2>/dev/null | awk -F: '/EPSG/{print $2}' | head -n 1)
  echo "${epsg:-NA} $f" >> "$EPSG_MAP"
done < "$RAW_LIST"
echo "[INFO] EPSG summary (top 20):"
awk '{print $1}' "$EPSG_MAP" | sort | uniq -c | sort -nr | head -n 20

echo "=== [${STATE}] Step2: per-file Warped VRT -> ${TGT_EPSG} ==="
export WARP_DIR TGT_EPSG TR NODATA
cat "$RAW_LIST" | xargs -n 1 -P "$P" -I {} bash -lc '
  f="{}"
  b=$(basename "$f")
  b="${b%.*}"
  out="${WARP_DIR}/${b}_5070.vrt"
  if [ -s "$out" ]; then exit 0; fi

  # Use 1 thread per process; we parallelize with xargs -P
  gdalwarp -of VRT \
    -t_srs "${TGT_EPSG}" \
    -tr ${TR} -tap \
    -r bilinear \
    -dstnodata "${NODATA}" \
    -multi -wo NUM_THREADS=1 \
    "$f" "$out" \
    > /dev/null
'

echo "[INFO] warpvrt count = $(ls -1 "$WARP_DIR"/*.vrt 2>/dev/null | wc -l)"

echo "=== [${STATE}] Step3: build state mosaic VRT ==="
gdalbuildvrt -overwrite "$MOSAIC_VRT" "$WARP_DIR"/*.vrt
echo "[INFO] mosaic vrt: $MOSAIC_VRT"

echo "=== [${STATE}] Step4: RANDOM sampling ${PATCH}x${PATCH} GeoTIFF tiles (target=N_TILES) ==="

N_TILES=${N_TILES:-20000}     # 你可以在运行前 export N_TILES=50000
MAX_TRIES=${MAX_TRIES:-400000} # 尝试次数上限（避免死循环）
