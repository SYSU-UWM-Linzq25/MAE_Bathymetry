...
def main():
    ...
    ap.add_argument("--loss_region_mode", choices=["all", "core"], default="all")
    ...
    ap.add_argument("--max_core_prediction_patch_ratio", type=float, default=0.50)
    args = ap.parse_args()
    ...
    for dem_path, mask_path in zip(dem_files, mask_files):
        row = {
            ...
            "ignored_patch_ratio": "",
            "core_valid_patch_ratio": "",
            "core_prediction_patch_ratio": "",
            "core_valid_patch_count": "",
            "core_prediction_patch_count": "",
            "status": "",
            "reason": "",
        }
        try:
            ...
            core = _core_patch_metrics(st, radius=args.core_patch_radius)
            core_valid_r = float(core["core_valid_patch_ratio"])
            core_pred_r = float(core["core_prediction_patch_ratio"])
            core_valid_n = int(core["core_valid_patch_count"])
            core_pred_n = int(core["core_prediction_patch_count"])
            ...
            elif args.loss_region_mode == "core" and (
                core_valid_r < args.min_core_valid_patch_ratio
            ):
                reason = "CORE_VALID_PATCH_RATIO"
            elif args.loss_region_mode == "core" and core_pred_n == 0:
                reason = "NO_CORE_PREDICTION_PATCH"
            elif args.loss_region_mode == "core" and not (
                args.min_core_prediction_patch_ratio
                <= core_pred_r
                <= args.max_core_prediction_patch_ratio
            ):
                reason = "CORE_PREDICTION_PATCH_RATIO"
            ...
            row.update({
                ...
                "core_valid_patch_ratio": "{:.8f}".format(core_valid_r),
                "core_prediction_patch_ratio": "{:.8f}".format(core_pred_r),
                "core_valid_patch_count": str(core_valid_n),
                "core_prediction_patch_count": str(core_pred_n),
