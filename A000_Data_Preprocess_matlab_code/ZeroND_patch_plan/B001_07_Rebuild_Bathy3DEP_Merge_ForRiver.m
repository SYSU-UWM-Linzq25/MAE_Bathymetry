function outVrt = B001_07_Rebuild_Bathy3DEP_Merge_ForRiver(river, varargin)
%% Rebuild bathy-priority + 3DEP merge for one river using canonical bathy.
%
% Correct merge logic:
%   1) valid bathy wins;
%   2) where bathy is NoData, valid 3DEP fills;
%   3) where both are invalid, output is -999999.
...
addParameter(p, 'bathyRootPattern', 'Bathy_%dm_FixND_ZeroFixed', @(x) ischar(x) || isstring(x));
addParameter(p, 'demRootPattern', '3DEP_%dm_ResampleClip', @(x) ischar(x) || isstring(x));
addParameter(p, 'outRootPattern', 'Bathy3DEP_Merged_Tiff_%dm_NDfix', @(x) ischar(x) || isstring(x));
...
% Existing 1 m folder uses this exact DEM filename.
...
if exist(bathyVrt, 'file') ~= 2; error('Missing bathy: %s', bathyVrt); end
if exist(demVrt, 'file') ~= 2; error('Missing 3DEP: %s', demVrt); end
...
if rowsB ~= rowsD || colsB ~= colsD
    error('Grid size mismatch: bathy=%d/%d, DEM=%d/%d', rowsB, colsB, rowsD, colsD);
end
if max(abs(geoB(:) - geoD(:))) > 1e-8
    error('GeoTransform mismatch between bathy and DEM.');
end
if ~strcmp(projB, projD)
    warning('Projection strings differ. Grids match, but inspect projection metadata.');
end
...
for rLocal = 1:tileSize:rowsB
    ...
    for cLocal = 1:tileSize:colsB
        ...
        if p.Results.assertNoZero && any(zeroOutput(:))
            error(['Unexpected zero remains in fixed merge: river=%s, block row=%d col=%d, ' ...
                   'N_zero=%d. Inspect bathy/DEM before continuing.'], ...
...
T = table(string(river), res, rowsB, colsB, N_total, N_bathy_valid, ...
    N_dem_fill, N_both_invalid, N_zero_output, validMin, validMax, globalND, ...
    'VariableNames', {'River','Resolution_m','Rows','Cols','N_total', ...
    'N_bathy_valid','N_filled_from_3DEP','N_both_invalid','N_zero_output', ...
