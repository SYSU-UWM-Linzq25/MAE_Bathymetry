function outVrt = B003s_NDfix_Build_SimpleFinalMask_ForRiver(river, varargin)
%% Build final MAE mask from LCC and canonical zero-fixed bathymetry.
%
% final_mask = (LCC == 1) & bathy_valid
% bathy_valid excludes NaN, Inf, -999999, and (optionally) zero-fill.
...
addParameter(p, 'outRootPattern', 'PredictionMask_LCCBathyValid_%dm_NDfix', @(x) ischar(x) || isstring(x));
addParameter(p, 'globalND', -999999, @isnumeric);
addParameter(p, 'maskND', 255, @isnumeric);
addParameter(p, 'zeroIsNoDataFallback', true, @islogical);
...
if rowsB ~= rowsL || colsB ~= colsL
    error('Grid size mismatch: bathy=%d/%d LCC=%d/%d', rowsB, colsB, rowsL, colsL);
end
if max(abs(geoB(:) - geoL(:))) > 1e-8
    error('GeoTransform mismatch between bathy and LCC.');
end
