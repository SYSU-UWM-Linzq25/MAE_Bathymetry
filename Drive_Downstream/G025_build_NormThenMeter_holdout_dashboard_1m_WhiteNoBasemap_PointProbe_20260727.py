#!/usr/bin/env python3
# NUMBER-ALIGNED NEW FAMILY COPY: G025_build_NormThenMeter_holdout_dashboard_1m_WhiteNoBasemap_PointProbe_20260727.py
# TEMPLATE SOURCE: G030_build_MeterOnly_holdout_dashboard_1m_WhiteNoBasemap_PointProbe_20260714.py
# NUMBER-ALIGNED NAME: G030_build_MeterOnly_holdout_dashboard_1m_WhiteNoBasemap_PointProbe_20260714.py
# ORIGINAL BACKUP NAME: G025_build_meterMAE_holdout_dashboard_1m_WhiteNoBasemap_PointProbe_F051Stable_20260714.py
# Compatibility rule: scientific logic and default data/result roots are preserved unless explicitly noted.
"""G025: NormThenMeter dashboard copied from the known-successful F051 implementation.
...
G025 creates local XYZ raster tiles and displays them in Leaflet over an
...
Experiments (allowlisted by default):
  - holdout_CA_D005Stage2NormThenMeter_FromNorm_D001NoDataSafe
  - holdout_CO_D005Stage2NormThenMeter_FromNorm_D001NoDataSafe
  - holdout_Santiam_D005Stage2NormThenMeter_FromNorm_D001NoDataSafe
...
* The official F062 unique-geospatial metrics are read from the existing summary JSON.  They are not
  recomputed from display tiles. Original F060/F062 rasters are never modified.
...
DEFAULT_OUT_DIR = Path(
    "/tank/data/SFS/xinyis/data/bathymetry/Processed_Results/"
    "FullRiver_WebMap_G025_NormThenMeter_Holdout_1m_WhiteNoBasemap_Probe"
)
DEFAULT_HTML_NAME = "G025_meterMAE_dashboard_1m_white_probe_F051Stable.html"
DEFAULT_ZIP_NAME = "G025_meterMAE_dashboard_1m_white_probe_F051Stable_package.zip"
DEFAULT_EXPERIMENTS = (
    "holdout_CA_D005Stage2NormThenMeter_FromNorm_D001NoDataSafe",
    "holdout_CO_D005Stage2NormThenMeter_FromNorm_D001NoDataSafe",
    "holdout_Santiam_D005Stage2NormThenMeter_FromNorm_D001NoDataSafe",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        ...
        description="Create the NormThenMeter holdout dashboard using the known-successful F051 web-map implementation.",
    )
    ...
    for experiment in experiments:
        ...
        for river in rivers:
            try:
                ...
                pred_summary_path = find_summary_json(pred_dir, ["F060_summary.json"])
                error_summary_path = find_summary_json(err_dir, ["F062_summary.json", "F062_gt_error_summary.json", "summary.json"])
...
def make_xyz_tiles_maxzoom_parent_pyramid(
    ...
    F060/F062 source rasters, and official metrics are not calculated here.
    """
    if out_dir.exists():
...
def extract_metrics(summary: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "n_pixels": first_metric(
            flat,
            [
                "unique_geospatial_n_pixels",
                "tile_footprint_weighted_n_pixels",
                "n_pixels",
                "valid_n_pixels",
            ],
            ["unique", "geospatial", "pixel"],
        ),
        "rmse_m": first_metric(
            flat,
            [
                "unique_geospatial_rmse_m",
                "tile_footprint_weighted_rmse_m",
                "rmse_m",
                "rmse",
            ],
            ["unique", "geospatial", "rmse"],
        ),
        "mae_m": first_metric(
            flat,
            [
                "unique_geospatial_mae_m",
                "tile_footprint_weighted_mae_m",
                "mae_m",
                "mae",
            ],
            ["unique", "geospatial", "mae"],
        ),
        "bias_m": first_metric(
            flat,
            [
                "unique_geospatial_bias_m",
                "tile_footprint_weighted_bias_m",
                "bias_m",
                "bias",
            ],
            ["unique", "geospatial", "bias"],
        ),
        "metric_scope": "F062 unique geospatial pixels after overlap averaging",
    }
...
def infer_error_definition(summary: Dict[str, Any]) -> str:
    ...
    return "Prediction - GT (F062 signed error)"
...
def process_one(
    ...
    # The manually written F060/F062 VRTs may not carry a usable <SRS>.
    ...
    if not src.error_vrt:
        raise RuntimeError(
            "F062 signed-error VRT is required by the core-GDAL version; "
    ...
    # Small samples are only for robust display limits; official metrics remain from F062 JSON.
    ...
    for layer_name in ("prediction", "gt", "error", "abs_error", "input_bathy", "hidden", "loss"):
        ...
        marker = layer_tiles / ".G025_complete.json"
    ...
    for layer_name, (probe_source, probe_resampling, probe_nodata, threshold) in probe_specs.items():
        ...
        marker = probe_dir / ".G025_probe_complete.json"
        signature = {
            "version": 1,
    ...
    record = {
        ...
        "visualization_policy": {
            ...
            "official_metrics_source": "F062 unique_geospatial_* fields; each final overlap-averaged full-river pixel counted once",
...
<section class="summary">
  <div class="metrics">
    <div class="metric"><div class="name">UNIQUE FULL-RIVER PIXELS</div><div class="value" id="metricN">—</div></div>
...
<div class="displayNotice">
  <strong>DISPLAY-ONLY RESAMPLING:</strong> MAE raster overlays are reprojected to an intermediate EPSG:3857 display grid of <strong>{manifest.get("detail_res_m", "—")} m/pixel</strong>. The finest XYZ web zoom is <strong>{manifest.get("xyz_finest_res_m", "—")} m/pixel</strong> at the equator. The original F060/F062 rasters and prediction values are not modified. <strong>RMSE, MAE, Bias, and N Pixels come from the F062 unique-geospatial full-river summary, where each final overlap-averaged river pixel is counted once</strong> and are not recalculated from these display tiles.<br>
...
<div id="probePanel">
  ...
  <div class="probeNote">Probe values are sampled from the finest resampled XYZ display grid, not directly from the native source pixel. Headline metrics use the F062 unique-geospatial native-grid summary, not display-tile values.</div>
</div>
<script>
if (!window.L) {{
...
def write_readme(
    ...
    text = f"""G025 Meter-MAE Full-River High-Detail Dashboard
...
2. Source rasters remain unchanged
   The original F060/F062 VRT/TIFF files are read-only inputs. This script never
...
3. Official metrics
   ...
   existing F062 summary JSON at native/source resolution. They are NOT recomputed
...
6. Coordinate reference system validation
   ...
   to the manually written F060/F062 VRTs before reprojection. The script stops
...
For a smaller display package, rerun G011 with DETAIL_RES_M=2 or 4.
...
Archive name:
  {zip_name}
"""
    (out_dir / "README_G025_meterMAE_F051Stable.txt").write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    print("============================================================")
    print("G025 NormThenMeter dashboard copied from the known-successful F051 browser-safe implementation")
    ...
    manifest = {
        ...
        "official_metrics": {
            "source": "F062 unique_geospatial_* summary fields",
            "resolution": "native full-river geospatial pixel grid after overlap averaging",
    ...
    (out_dir / "G025_manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    write_readme(
        out_dir,
        args.out_html,
        args.zip_name,
