...
# ORIGINAL BACKUP NAME: F062_fullriver_gt_error_uniquePixel_MeterMAE_20260713.py
# Compatibility rule: scientific logic and default data/result roots are preserved unless explicitly noted.
"""F062: build GT/error GeoTIFFs and exact unique-pixel metrics for F060 meter-MAE full-river predictions.

Inputs
------
Per-river F060 TileAvgVRT outputs:
...
For each averaged prediction tile, this script reads:
  ...
  - F060 averaged prediction tile

Outputs
-------
Per river:
  gt_core_final_loss_tile/
    F062_gt_m_<key>_core_final_loss.tif
  error_signed_m_tile/
    F062_err_signed_m_<key>_core_final_loss.tif
  error_abs_m_tile/
    F062_err_abs_m_<key>_core_final_loss.tif

  F062_fullriver_gt_m_<river>_core_final_loss_tiles.vrt
  F062_fullriver_err_signed_m_<river>_core_final_loss_tiles.vrt
  F062_fullriver_err_abs_m_<river>_core_final_loss_tiles.vrt
  F062_tile_error_metrics.csv
  F062_summary.json
...
Hidden_Mask is not used in F062. It was only used in F060 model inference.
"""
...
def find_river_dirs(f060_out_dir: Path, rivers: Sequence[str]) -> List[Path]:
    ...
    if rivers:
        ...
        dirs = [p for p in sorted(f060_out_dir.iterdir()) if p.is_dir() and (p / 'F060_summary.json').exists()]
    missing = [str(d) for d in dirs if not (d / 'F060_summary.json').exists()]
    if missing:
        raise FileNotFoundError('Missing F060_summary.json for river dirs:\n' + '\n'.join(missing))
    return dirs
...
def process_river(river_dir: Path, out_base: Path, args) -> Dict[str, Any]:
    ...
    summary_path = river_dir / 'F060_summary.json'
    manifest_path = river_dir / 'F060_tileavg_prediction_manifest.csv'
    if not manifest_path.exists():
        raise FileNotFoundError(f'Missing F060 manifest: {manifest_path}')
    ...
    for idx, r in enumerate(rows, start=1):
        ...
        gt_out_path = gt_dir / f'F062_gt_m_{key}_core_final_loss.tif'
        err_signed_path = err_signed_dir / f'F062_err_signed_m_{key}_core_final_loss.tif'
        err_abs_path = err_abs_dir / f'F062_err_abs_m_{key}_core_final_loss.tif'
    ...
    if not metric_rows:
        raise RuntimeError(f'No F062 outputs produced for river={river}.')

    gt_vrt = out_river / f'F062_fullriver_gt_m_{river}_core_final_loss_tiles.vrt'
    err_signed_vrt = out_river / f'F062_fullriver_err_signed_m_{river}_core_final_loss_tiles.vrt'
    err_abs_vrt = out_river / f'F062_fullriver_err_abs_m_{river}_core_final_loss_tiles.vrt'
    ...
    write_csv(out_river / 'F062_tile_error_metrics.csv', metric_rows)
    write_csv(out_river / 'F062_skipped_tiles.csv', skipped)
    ...
    (out_river / 'F062_summary.json').write_text(json.dumps(summary, indent=2))
...
def main() -> None:
    ...
    ap.add_argument('--f060_out_dir', required=True, help='F060 experiment output folder, e.g. holdout_CO_D001NoDataSafe')
    ...
    (out / 'F062_args.json').write_text(json.dumps({
        **vars(args),
        'resolved_river_dirs': [str(d) for d in river_dirs],
    }, indent=2))

    print('[F062] Build GT/error VRTs plus legacy tile-footprint and exact unique-pixel metrics')
    print(f'[F062] f060_out_dir={f060_out}')
    print(f'[F062] output_dir={out}')
    print(f'[F062] rivers={[d.name for d in river_dirs]}')

    summaries = []
    for d in river_dirs:
        summaries.append(process_river(d, out, args))

    (out / 'F062_all_rivers_summary.json').write_text(json.dumps(summaries, indent=2))
    print('[DONE]', out)
