...
# ORIGINAL BACKUP NAME: F060_fullriver_predict_tileavg_vrt_MeterMAE_CoreFinalLossOnly_Inst1DSafe_20260713.py
# Compatibility rule: scientific logic and default data/result roots are preserved unless explicitly noted.
"""F060 full-river inference using the v4 meter-MAE checkpoint by per-tile prediction + sparse overlap averaging + VRT.
...
def infer_river(model, items: Sequence[TileItem], river: str, args, device: torch.device) -> Dict[str, Any]:
    ...
    for rec in sparse_records:
        ...
        avg_path = avg_tile_dir / f"F060_avg_pred_m_{rec.key}_core_final_loss.tif"
        cnt_path = count_tile_dir / f"F060_overlap_count_{rec.key}_core_final_loss.tif"
    ...
    pred_vrt = out_river / f"F060_fullriver_pred_m_{river}_core_final_loss_avg_tiles.vrt"
    count_vrt = out_river / f"F060_fullriver_overlap_count_{river}_core_final_loss_avg_tiles.vrt"
    ...
    write_csv(out_river / "F060_tileavg_prediction_manifest.csv", manifest_rows)
    write_csv(out_river / "F060_skipped_tiles.csv", skipped)
    ...
    (out_river / "F060_summary.json").write_text(json.dumps(summary, indent=2))
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "F060_summary.json and both VRT outputs are complete; otherwise "
    ...
    if not TIFFFILE_AVAILABLE:
        raise RuntimeError("tifffile is required for this F060 tile-average VRT workflow.")
    ...
    args_file = out / "F060_args.json"
    if not args_file.exists():
        args_file.write_text(json.dumps(args_payload, indent=2))
    (out / "F060_args_last_invocation.json").write_text(json.dumps(args_payload, indent=2))
    ...
    for river in rivers:
        river_out = out / river
        summary_path = river_out / "F060_summary.json"
    ...
    for summary_path in sorted(out.glob("*/F060_summary.json")):
        summary = completed_summary(summary_path)
        if summary is not None:
            all_completed.append(summary)
    (out / "F060_all_rivers_summary.json").write_text(json.dumps(all_completed, indent=2))
