#!/usr/bin/env bash
# NUMBER-ALIGNED NAME: B010q2_run_source_chain_gis_export_for_suspicious_tiles.sh
# ORIGINAL BACKUP NAME: B014_run_source_chain_gis_export_for_suspicious_tiles.sh
...
SCRIPT=${SCRIPT:-$WORK/script/A010q2_inspect_suspicious_tile_source_chain_gis_export.py}
...
if ! command -v gdaltransform >/dev/null 2>&1; then
  echo "[ERROR] gdaltransform not found in PATH." >&2
  echo "        Try: module avail gdal" >&2
  echo "        Then rerun with: GDAL_MODULE=<module_name> bash $(basename "$0")" >&2
  exit 5
fi
...
echo "[GDAL] gdaltransform=$(command -v gdaltransform)"
...
echo "=== B014 source-chain exact-mask GIS exporter ==="
echo "SCRIPT=$SCRIPT"
