#!/usr/bin/env bash
#SBATCH -J D026_s2_meter_from_norm
#SBATCH -p HydroIntel
...
CV_ROOT=${CV_ROOT:-$WORK/cross_validation_v6_Stage2MeterMAE_FromNorm}

SOURCE_SPLIT_TAG=${SOURCE_SPLIT_TAG:-D001NoDataSafe}
RUN_TAG=${RUN_TAG:-D005Stage2MeterMAE_FromNorm_D001NoDataSafe}
TRAIN_BACKEND=${TRAIN_BACKEND:-$WORK/script/D025_train_stage2_MeterMAE_FromNorm_backend_20260725.sh}
...
exec >"$RUNTIME_LOG_DIR/D026_stage2_meter_from_norm_${HOLDOUT_PRESET}_${RUNTIME_JOB_ID}.out" \
     2>"$RUNTIME_LOG_DIR/D026_stage2_meter_from_norm_${HOLDOUT_PRESET}_${RUNTIME_JOB_ID}.err"
...
case "$HOLDOUT_PRESET" in
  ...
  Santiam)
    ...
    ;;
  *)
    echo "[ERROR] D026 formal submission currently supports CA, CO, and Santiam. Got $HOLDOUT_PRESET" >&2
    exit 2
    ;;
esac
...
echo "============================================================"
echo "D026 holdout Stage-2 meter-MAE optimization from normalized-loss best"
date
