...
# D025: MAE v2 dual-mask downstream training with meter-space MAE objective.
#
# Inputs:
...
echo "=== D025 MAE v2 dual-mask meter-MAE training backend ==="
echo "HOST=$(hostname)"
echo "ROOT=$ROOT"
echo "CODE=$CODE"
...
for f in \
  "$CODE/main_pretrain_dem_meterMAE_D025_20260712.py" \
  "$CODE/engine_pretrain_meterMAE_D025_20260712.py" \
...
python "$CODE/main_pretrain_dem_meterMAE_D025_20260712.py" --help > "$OUT_DIR/help.txt" 2>&1 || true
for key in \
  ...
  optimization_loss early_stop_patience early_stop_metric best_metric val_mae_m_mask eval_rmse; do
...
PYTHONUNBUFFERED=1 python -u main_pretrain_dem_meterMAE_D025_20260712.py \
  --data_root "$TILE_ROOT" \
...
date
echo "=== D025 MAE v2 dual-mask meter-MAE training done ==="
echo "RUN=$OUT_DIR"
