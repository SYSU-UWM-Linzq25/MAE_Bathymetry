#!/bin/bash
# NUMBER-ALIGNED NAME: E003a_eval_stage4_best_val_nodata_core_tilenormVis.sh
# ORIGINAL BACKUP NAME: E021_eval_stage4_best_val_nodata_core_tilenormVis.sh
# Compatibility rule: scientific logic and default data/result roots are preserved unless explicitly noted.
#SBATCH -J eval_s4_core_val
#SBATCH -p gpu
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -c 8
#SBATCH -t 08:00:00
#SBATCH -o /tank/data/SFS/xinyis/data/bathymetry/MAE-Topography/Downstream_Task_Bathy/logs/eval_s4_core_val_%j.out
#SBATCH -e /tank/data/SFS/xinyis/data/bathymetry/MAE-Topography/Downstream_Task_Bathy/logs/eval_s4_core_val_%j.out
...
SCRIPT=$WORK/script/E003_evaluate_stage4_nodata_core_per_tile.py
...
RUN_NAME=stage4_bathy_finalmask_holdout_${SAFE_VAL}_exact_freeze_decoder_tilenormVis_nodataSafe_${LOSS_REGION_MODE}Loss_r${CORE_PATCH_RADIUS}_cv${MIN_CORE_VALID_PATCH_RATIO//./p}_cp${MIN_CORE_PREDICTION_PATCH_RATIO//./p}-${MAX_CORE_PREDICTION_PATCH_RATIO//./p}_std${STD_SCALE//./p}_1m_e${END_EPOCH}
RUN=$WORK/runs/$RUN_NAME
CKPT=${CKPT:-$RUN/checkpoint-best.pth}
OUT=$RUN/eval_val_nodata_core_exact_pixel
