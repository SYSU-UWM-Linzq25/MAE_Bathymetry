...
# D025: Stage-2 dual-mask fine-tuning backend.
#
...
RUN_NAME=${RUN_NAME:-stage2_meterMAE_from_normalized_best}
...
if [[ "$OPTIMIZATION_LOSS" != "meter_mae" ]]; then
  echo "[ERROR] D025 Stage 2 requires OPTIMIZATION_LOSS=meter_mae, got $OPTIMIZATION_LOSS" >&2
  exit 2
fi
if [[ "$BEST_METRIC" != "val_mae_m_mask" || "$EARLY_STOP_METRIC" != "val_mae_m_mask" ]]; then
  echo "[ERROR] D025 safety rule requires BEST_METRIC=EARLY_STOP_METRIC=val_mae_m_mask" >&2
  exit 2
fi
...
echo "============================================================"
echo "D025 Stage-2 meter-MAE optimization from normalized-loss checkpoint"
date
echo "HOST=$(hostname)"
...
echo "============================================================"
echo "DONE D025 Stage 2 normalized -> meter"
echo "RUN=$OUT_DIR"
date
echo "============================================================"
