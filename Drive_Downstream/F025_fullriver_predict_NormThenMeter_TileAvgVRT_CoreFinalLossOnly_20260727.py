#!/usr/bin/env python3
# NUMBER-ALIGNED NEW FAMILY COPY: F025_fullriver_predict_NormThenMeter_TileAvgVRT_CoreFinalLossOnly_20260727.py
# TEMPLATE SOURCE: F030_fullriver_predict_MeterOnly_TileAvgVRT_CoreFinalLossOnly_20260713.py
# Writes the legacy F060 manifest/summary product names for downstream compatibility.
# NUMBER-ALIGNED NAME: F030_fullriver_predict_MeterOnly_TileAvgVRT_CoreFinalLossOnly_20260713.py
# ORIGINAL BACKUP NAME: F025_fullriver_predict_tileavg_vrt_MeterMAE_CoreFinalLossOnly_Inst1DSafe_20260713.py
# Compatibility rule: scientific logic and default data/result roots are preserved unless explicitly noted.
"""F025 full-river inference using the v4 NormThenMeter checkpoint by per-tile prediction + sparse overlap averaging + VRT.
...
def infer_river(model, items: Sequence[TileItem], river: str, args, device: torch.device) -> Dict[str, Any]:
    ...
    for rec in sparse_records:
        ...
        avg_path = avg_tile_dir / f"F025_avg_pred_m_{rec.key}_core_final_loss.tif"
        cnt_path = count_tile_dir / f"F025_overlap_count_{rec.key}_core_final_loss.tif"
    ...
    pred_vrt = out_river / f"F025_fullriver_pred_m_{river}_core_final_loss_avg_tiles.vrt"
    count_vrt = out_river / f"F025_fullriver_overlap_count_{river}_core_final_loss_avg_tiles.vrt"
    ...
    write_csv(out_river / "F025_tileavg_prediction_manifest.csv", manifest_rows)
    write_csv(out_river / "F025_skipped_tiles.csv", skipped)
    ...
    (out_river / "F025_summary.json").write_text(json.dumps(summary, indent=2))
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "F025_summary.json and both VRT outputs are complete; otherwise "
    ...
    if not TIFFFILE_AVAILABLE:
        raise RuntimeError("tifffile is required for this F025 tile-average VRT workflow.")
    ...
    args_file = out / "F025_args.json"
    if not args_file.exists():
        args_file.write_text(json.dumps(args_payload, indent=2))
    (out / "F025_args_last_invocation.json").write_text(json.dumps(args_payload, indent=2))
    ...
    for river in rivers:
        river_out = out / river
        summary_path = river_out / "F025_summary.json"
    ...
    for summary_path in sorted(out.glob("*/F025_summary.json")):
        summary = completed_summary(summary_path)
        if summary is not None:
            all_completed.append(summary)
    (out / "F025_all_rivers_summary.json").write_text(json.dumps(all_completed, indent=2))
