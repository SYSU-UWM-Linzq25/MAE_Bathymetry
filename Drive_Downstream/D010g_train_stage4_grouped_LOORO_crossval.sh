...
HOLDOUT_RIVERS_FILE="$SPLIT/holdout_rivers.txt"
python - "$SPLIT/train.txt" "$SPLIT/val.txt" "$VAL_RIVER" "$HOLDOUT_RIVERS_FILE" <<'PY'
import re
import sys
...
val_river = sys.argv[3]
holdout_file = Path(sys.argv[4])

if holdout_file.is_file():
    holdouts = [line.strip() for line in holdout_file.read_text().splitlines() if line.strip()]
else:
    holdouts = [val_river]

tile_re = re.compile(r"^Select_tile_Basin_1m_(.+)_ID\d+\.tif$")

def parse_river(line: str) -> str:
...
train_rivers = [parse_river(x) for x in train_file.read_text().splitlines() if x.strip()]
val_rivers = [parse_river(x) for x in val_file.read_text().splitlines() if x.strip()]

bad_train = sorted(set(train_rivers) & holdout_set)
bad_val = sorted(set(val_rivers) - holdout_set)

print("[HOLDOUT RIVERS] " + ", ".join(holdouts))
print("[TRAIN RIVER COUNT] total_rivers=", len(set(train_rivers)), "train_tiles=", len(train_rivers))
print("[VAL RIVER COUNT] total_rivers=", len(set(val_rivers)), "val_tiles=", len(val_rivers))
for r in sorted(set(val_rivers)):
    print(f"  val {r}: {val_rivers.count(r)}")

if bad_train:
    print("[ERROR] Holdout river(s) appear in train.txt:", bad_train, file=sys.stderr)
    sys.exit(4)
if bad_val:
    print("[ERROR] val.txt contains non-holdout river(s):", bad_val, file=sys.stderr)
    sys.exit(5)
PY
