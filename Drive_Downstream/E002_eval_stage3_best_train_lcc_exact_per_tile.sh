#!/bin/bash
#SBATCH -J eval_s3_train_lcc
#SBATCH -p gpu
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -c 10
#SBATCH -t 12:00:00
#SBATCH -o /tank/data/SFS/xinyis/data/bathymetry/MAE-Topography/Downstream_Task_Bathy/logs/eval_s3_train_%j.out
#SBATCH -e /tank/data/SFS/xinyis/data/bathymetry/MAE-Topography/Downstream_Task_Bathy/logs/eval_s3_train_%j.out
...
OUT=$WORK/eval_stage3_bathy_lcc_exact_best/train_best_fullnorm_top100
mkdir -p "$OUT" "$WORK/logs"

export PYTHONPATH="$CODE${PYTHONPATH:+:$PYTHONPATH}"

echo "=== Stage3 LCC exact per-tile TRAIN evaluation ${SLURM_JOB_ID:-local} on $(hostname) ==="
date
echo "CODE=$CODE"
echo "CKPT=$CKPT"
echo "SPLIT=$SPLIT"
