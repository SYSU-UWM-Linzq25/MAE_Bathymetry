...
#SBATCH -c 8
#SBATCH -t 7-00:00:00
#SBATCH -o /tank/data/SFS/xinyis/data/bathymetry/MAE-Topography/Downstream_Task_Bathy/logs/train_s4_allRiverCanonicalND_core_vis_%j.out
#SBATCH -e /tank/data/SFS/xinyis/data/bathymetry/MAE-Topography/Downstream_Task_Bathy/logs/train_s4_allRiverCanonicalND_core_vis_%j.err
...
# A004 audit generated after publishing the CanonicalND products for all
# retained train and holdout rivers. Training reads train.txt/val.txt
# directly; these CSVs are required as the reproducibility record.
AUDIT_DIR=$WORK/splits/nodata_core_loss_audit_allRiverCanonicalND
TRAIN_AUDIT=$AUDIT_DIR/train_core_loss_audit_allRiverCanonicalND.csv
VAL_AUDIT=$AUDIT_DIR/val_core_loss_audit_allRiverCanonicalND.csv
...
echo "TRAIN_AUDIT=$TRAIN_AUDIT"
echo "VAL_AUDIT=$VAL_AUDIT"
echo "BATH=$BATH"
echo "MASK=$MASK"
...
echo "MIN_VISIBLE_PATCH_RATIO=$MIN_VISIBLE_PATCH_RATIO"
...
for key in \
  ...
  tile_norm_visible_only tile_norm_std_scale nodata nodata_threshold min_valid_visible_patch_ratio \
  loss_region_mode core_patch_radius min_core_valid_patch_ratio \
  min_core_prediction_patch_ratio max_core_prediction_patch_ratio; do
...
echo "=== All-river CanonicalND A004 audit summary ==="
...
for label, filename in zip(("train", "val"), sys.argv[1:]):
    ...
    with path.open(newline="") as f:
        for row in csv.DictReader(f):
            ...
            key = reason if reason and reason != "PASS" else status
            counts[key or "UNKNOWN"] += 1

    passed = counts.get("PASS", 0)
    print(f"[AUDIT] {label}: total={total} pass={passed} drop={total-passed}")
    print(f"[AUDIT] {label} reasons={dict(sorted(counts.items()))}")
PY

# Preserve the exact audits used for this run.
cp -pf "$TRAIN_AUDIT" "$OUT/input_train_core_loss_audit_allRiverCanonicalND.csv"
cp -pf "$VAL_AUDIT" "$OUT/input_val_core_loss_audit_allRiverCanonicalND.csv"
...
# 5. Training
# ==============================
# allRiverCanonicalND input: all retained train/holdout rivers use the same
# explicit Float32 NoData=-999999 and NoData-safe Bathy+3DEP merge chain.
# NoData-safe patch rules: any NoData pixel invalidates the full 16x16 patch.
# Invalid patches are excluded from encoder, decoder attention, loss, RMSE, and output.
# The full valid tile remains context: every valid known patch can enter the encoder.
# Every usable river patch across the full tile remains hidden/predicted.
# Only loss/RMSE can be restricted to the centered core through LOSS_REGION_MODE.
