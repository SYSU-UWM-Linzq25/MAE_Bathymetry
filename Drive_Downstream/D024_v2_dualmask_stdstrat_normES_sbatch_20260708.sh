...
#SBATCH --mem=48G
#SBATCH -t 7-00:00:00
#SBATCH -o /tank/data/SFS/xinyis/data/bathymetry/MAE-Topography/Downstream_Task_Bathy/cross_validation_v2/logs/D024_stdstrat_normES_%j.out
#SBATCH -e /tank/data/SFS/xinyis/data/bathymetry/MAE-Topography/Downstream_Task_Bathy/cross_validation_v2/logs/D024_stdstrat_normES_%j.err
...
# D024 clean: make high/mid/low tile_std_safe river split, then train using normalized early stopping.
#
# Submit example:
...
if [[ -z "${RUN_STAGE:-}" ]]; then
  if [[ "$EPOCHS" -le 5 ]]; then
    RUN_STAGE="smoke_stdstrat"
  else
    RUN_STAGE="train_stdstrat"
  fi
fi

METRIC_TAG=$(echo "$EARLY_STOP_METRIC" | sed 's/[^A-Za-z0-9_]/_/g')
SPLIT_NAME="stdStratRiver_${BIN_STAT}_valPerBin${VAL_PER_BIN}_seed${STD_SPLIT_SEED}"
