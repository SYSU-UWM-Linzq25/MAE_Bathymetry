...
# D020 v2 final: training backend for MAE v2 dual-mask downstream training.
#
# Inputs:
...
RUN_NAME=${RUN_NAME:-stage4_v2_dualmask_decoder_tilenormVis_pixelLoss_coreLoss_1m_e400}
...
echo "=== D020 MAE v2 dual-mask training backend ==="
echo "HOST=$(hostname)"
echo "ROOT=$ROOT"
echo "CODE=$CODE"
...
python "$CODE/main_pretrain_dem.py" --help > "$OUT_DIR/help.txt" 2>&1 || true
for key in \
  ...
  if ! grep -q -- "$key" "$OUT_DIR/help.txt"; then
    echo "[ERROR] main_pretrain_dem.py does not support expected argument containing: $key" >&2
...
PYTHONUNBUFFERED=1 python -u main_pretrain_dem.py \
  --data_root "$TILE_ROOT" \
...
date
echo "=== D020 MAE v2 dual-mask training done ==="
echo "RUN=$OUT_DIR"
