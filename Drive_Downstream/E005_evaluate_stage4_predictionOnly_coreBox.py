#!/usr/bin/env python3
# NUMBER-ALIGNED NAME: E005_evaluate_stage4_predictionOnly_coreBox.py
# ORIGINAL BACKUP NAME: E005_evaluate_stage4_predictionOnly_coreBox.py
...
def _core_region_mask_and_bounds(
    height: int,
    width: int,
    patch_size: int,
    core_patch_radius: int,
) -> Tuple[np.ndarray, Tuple[int, int, int, int]]:
    """Return centered core mask and (x0, y0, x1, y1) pixel bounds."""
    if height % patch_size != 0 or width % patch_size != 0:
        raise ValueError(
            f"Image size {height}x{width} is not divisible by patch size {patch_size}."
        )
    ...
    row0_patch = max(0, center_row - core_patch_radius)
    row1_patch = min(n_rows, center_row + core_patch_radius + 1)
    col0_patch = max(0, center_col - core_patch_radius)
    col1_patch = min(n_cols, center_col + core_patch_radius + 1)

    y0 = row0_patch * patch_size
    y1 = row1_patch * patch_size
    x0 = col0_patch * patch_size
    x1 = col1_patch * patch_size
...
def _robust_joint_limits(*arrays: np.ndarray) -> Tuple[float, float]:
    values = []
    for arr in arrays:
    ...
    vmin = float(np.percentile(joined, 1))
    vmax = float(np.percentile(joined, 99))
...
def _quicklook_prediction_only(
    output: Path,
    gt_prediction_only: np.ndarray,
    pred_prediction_only: np.ndarray,
    error_prediction_only: np.ndarray,
    core_bounds: Tuple[int, int, int, int],
    title: str,
) -> None:
    """Plot only predicted pixels; known/context pixels remain blank.

    Top row: full tile, with the fixed core outlined.
    Bottom row: zoomed core.
    GT and prediction share the same elevation limits.
    """
    try:
        import matplotlib.pyplot as plt
        from matplotlib.patches import Rectangle

        gt = np.asarray(gt_prediction_only, dtype=np.float64)
        pred = np.asarray(pred_prediction_only, dtype=np.float64)
        err = np.asarray(error_prediction_only, dtype=np.float64)

        vmin, vmax = _robust_joint_limits(gt, pred)
        valid_error = err[np.isfinite(err)]
        err_vmax = (
            float(np.percentile(np.abs(valid_error), 98))
            if valid_error.size
            else 1.0
        )
        ...
        gt_core = gt[y0:y1, x0:x1]
        pred_core = pred[y0:y1, x0:x1]
        err_core = err[y0:y1, x0:x1]

        fig, axes = plt.subplots(2, 3, figsize=(15, 10))
        top_panels = [
            (gt, "GT — predicted patches only", (vmin, vmax)),
            (pred, "Prediction — predicted patches only", (vmin, vmax)),
            (err, "Error — predicted patches only", (-err_vmax, err_vmax)),
        ]
        bottom_panels = [
            (gt_core, "GT — core zoom", (vmin, vmax)),
            (pred_core, "Prediction — core zoom", (vmin, vmax)),
            (err_core, "Error — core zoom", (-err_vmax, err_vmax)),
        ]

        for ax, (arr, name, limits) in zip(axes[0], top_panels):
        ...
        for ax, (arr, name, limits) in zip(axes[1], bottom_panels):
            im = ax.imshow(arr, vmin=limits[0], vmax=limits[1])
            ax.set_title(name)
            ax.axis("off")
            fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
        ...
        print(f"[WARN] prediction-only quicklook failed for {output}: {exc}")
...
def _save_selected_visuals(
    ...
    for rank, row in enumerate(rows, start=1):
        ...
        loss_img = (
            _expand_patch_mask(model, loss_mask.float(), args.in_chans)[0, 0]
        ...
        core_region, core_bounds = _core_region_mask_and_bounds(
            gt_m.shape[0],
            gt_m.shape[1],
            patch_size,
            args.core_patch_radius,
        )

        prediction_valid = prediction_img & valid_np
        core_prediction_valid = prediction_valid & core_region
        outer_prediction_valid = prediction_valid & ~core_region

        exact_core = lcc_np & core_prediction_valid
        exact_outer = lcc_np & outer_prediction_valid
        exact_full = exact_core | exact_outer

        # Only predicted pixels are shown in the quicklook. Known/context
        # pixels are NaN and are therefore visually blank.
        gt_prediction_only = np.where(prediction_valid, gt_m, np.nan)
        pred_prediction_only = np.where(prediction_valid, pred_m, np.nan)
        err_prediction_only = np.where(prediction_valid, err, np.nan)

        # Retain a legacy reconstruction GeoTIFF for compatibility, but do
        # not include it in the PNG quicklook.
        reconstruction = gt_m.copy()
        reconstruction[prediction_valid] = pred_m[prediction_valid]

        gt_out = np.where(valid_np, gt_m, nodata_out).astype(np.float32)
        gt_prediction_out = np.where(
            prediction_valid, gt_m, nodata_out
        ).astype(np.float32)
        pred_prediction_out = np.where(
            prediction_valid, pred_m, nodata_out
        ).astype(np.float32)
        err_prediction_out = np.where(
            prediction_valid, err, nodata_out
        ).astype(np.float32)

        gt_core_out = np.where(
            core_prediction_valid, gt_m, nodata_out
        ).astype(np.float32)
        pred_core_out = np.where(
            core_prediction_valid, pred_m, nodata_out
        ).astype(np.float32)
        err_core_patch_out = np.where(
            core_prediction_valid, err, nodata_out
        ).astype(np.float32)

        err_core_exact = np.where(
            exact_core, err, nodata_out
        ).astype(np.float32)
        err_outer_exact = np.where(
            exact_outer, err, nodata_out
        ).astype(np.float32)
        err_full_exact = np.where(
            exact_full, err, nodata_out
        ).astype(np.float32)
        recon_out = np.where(
            valid_np, reconstruction, nodata_out
        ).astype(np.float32)

        metric = _safe_float(row.get(rank_key))
        metric_text = "nan" if metric is None else f"{metric:.3f}"
        sample_dir = out_root / (
            f"rank{rank:03d}_idx{index:06d}_{rank_key}{metric_text}"
        )
        sample_dir.mkdir(parents=True, exist_ok=True)

        outputs = [
            ("gt_all_valid_m.tif", gt_out, "float32", nodata_out),
            ("gt_prediction_only_m.tif", gt_prediction_out, "float32", nodata_out),
            ("pred_prediction_only_m.tif", pred_prediction_out, "float32", nodata_out),
            ("err_prediction_only_m.tif", err_prediction_out, "float32", nodata_out),
            ("gt_core_prediction_only_m.tif", gt_core_out, "float32", nodata_out),
            ("pred_core_prediction_only_m.tif", pred_core_out, "float32", nodata_out),
            ("err_core_prediction_patch_m.tif", err_core_patch_out, "float32", nodata_out),
            ("err_core_exact_pixel_m.tif", err_core_exact, "float32", nodata_out),
            ("err_outer_exact_pixel_m.tif", err_outer_exact, "float32", nodata_out),
            ("err_full_exact_pixel_m.tif", err_full_exact, "float32", nodata_out),
            ("reconstruction_context_legacy_m.tif", recon_out, "float32", nodata_out),
            ("final_mask_pixel.tif", lcc_np.astype(np.uint8), "uint8", 0),
            ("valid_pixel_mask.tif", valid_np.astype(np.uint8), "uint8", 0),
            ("core_region_box_mask.tif", core_region.astype(np.uint8), "uint8", 0),
            ("core_prediction_patch_mask.tif", core_prediction_valid.astype(np.uint8), "uint8", 0),
            ("outer_prediction_patch_mask.tif", outer_prediction_valid.astype(np.uint8), "uint8", 0),
            ("prediction_patch_mask.tif", prediction_img.astype(np.uint8), "uint8", 0),
        ]
        for filename, array, dtype, nodata in outputs:
            _write_geotiff_like(
                path, sample_dir / filename, array, dtype, nodata
            )

        with (sample_dir / "metrics.json").open("w") as f:
            json.dump(row, f, indent=2)

        core_rmse = _safe_float(row.get("rmse_m_core_exact_pixel"))
        outer_rmse = _safe_float(row.get("rmse_m_outer_exact_pixel"))
        full_rmse = _safe_float(row.get("rmse_m_full_exact_pixel"))
        metric_bits = [
            f"{rank_key}={metric_text} m",
            "core exact RMSE="
            + ("nan" if core_rmse is None else f"{core_rmse:.3f} m"),
            "outer exact RMSE="
            + ("nan" if outer_rmse is None else f"{outer_rmse:.3f} m"),
            "full exact RMSE="
            + ("nan" if full_rmse is None else f"{full_rmse:.3f} m"),
        ]

        _quicklook_prediction_only(
            sample_dir / "quicklook_prediction_only_core_box.png",
            gt_prediction_only,
            pred_prediction_only,
            err_prediction_only,
            core_bounds,
            title=f"{Path(path).name} | " + " | ".join(metric_bits),
        )
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        choices=["rmse_m_core_exact_pixel", "rmse_m_core_patch",
                 "rmse_m_outer_exact_pixel", "rmse_m_outer_prediction_patch",
    ...
    totals = {
        ...
        "outer_patch_sse": 0.0, "outer_patch_count": 0,
        "full_patch_sse": 0.0, "full_patch_count": 0,
        "core_exact_sse": 0.0, "core_exact_count": 0,
        "outer_exact_sse": 0.0, "outer_exact_count": 0,
    ...
    for batch_index, batch in enumerate(loader):
        ...
        for i in range(batch_size):
            ...
            core_region_np, _ = _core_region_mask_and_bounds(
                err_m.shape[0],
                err_m.shape[1],
                patch_size,
                args.core_patch_radius,
            ...
            totals["outer_patch_sse"] += outer_patch["sse"]
            totals["outer_patch_count"] += outer_patch["count"]
            ...
            totals["outer_exact_sse"] += outer_exact["sse"]
            totals["outer_exact_count"] += outer_exact["count"]
            ...
            row = {
                ...
                "rmse_m_outer_prediction_patch": outer_patch["rmse"],
                "mae_m_outer_prediction_patch": outer_patch["mae"],
                "bias_m_outer_prediction_patch": outer_patch["bias"],
                ...
                "rmse_m_outer_exact_pixel": outer_exact["rmse"],
                "mae_m_outer_exact_pixel": outer_exact["mae"],
                "bias_m_outer_exact_pixel": outer_exact["bias"],
                "max_abs_m_outer_exact_pixel": outer_exact["max_abs"],
                "p95_abs_m_outer_exact_pixel": outer_exact["p95_abs"],
                "rmse_m_full_exact_pixel": full_exact["rmse"],
                "n_core_patch_pixels": core_patch["count"],
                "n_outer_prediction_patch_pixels": outer_patch["count"],
                "n_core_exact_pixels": core_exact["count"],
                "n_outer_exact_pixels": outer_exact["count"],
    ...
    summary = {
        ...
        "global_pixel_weighted_rmse_m": {
            ...
            "outer_prediction_patch": global_rmse(
                "outer_patch_sse", "outer_patch_count"
            ),
            "full_prediction_patch": global_rmse(
                "full_patch_sse", "full_patch_count"
            ),
            "core_exact_final_mask_pixel": global_rmse(
                "core_exact_sse", "core_exact_count"
            ),
            "outer_exact_final_mask_pixel": global_rmse(
                "outer_exact_sse", "outer_exact_count"
            ),
        ...
        "per_tile_summary": {
            ...
            "rmse_m_outer_exact_pixel": _summary(
                [r["rmse_m_outer_exact_pixel"] for r in rows]
            ),
            "rmse_m_full_exact_pixel": _summary(
                [r["rmse_m_full_exact_pixel"] for r in rows]
            ),
            "rmse_m_outer_prediction_patch": _summary(
                [r["rmse_m_outer_prediction_patch"] for r in rows]
            ),
