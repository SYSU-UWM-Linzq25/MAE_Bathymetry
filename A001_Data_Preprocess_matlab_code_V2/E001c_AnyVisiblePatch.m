function E001c_AnyVisiblePatch(rivers, varargin)
% E001c_AnyVisiblePatch
%
% Full-river extraction derived directly from:
%   E001_SelectTiles_Overlap_FullRiver_ForRiver_V5_NoDataSafe
%
% This file uses the SAME teacher-agreed patch HiddenMask rule as the revised
% D001c training extractor.
%
% Pixel definitions:
%   hiddenPixel  = valid pixel marked hidden/bathy by the source HiddenMask.
%   visiblePixel = valid DEM pixel not marked hidden.
%
...
%
% Thus one valid visible pixel makes the whole patch visible. A patch is hidden
% only when it contains no valid visible pixel. For a fully valid patch this is
% equivalent to hiddenPatch = ALL(hiddenPixel).
%
...
%
% Output:
%   Processed_Results/Tiles_for_MAE_FullRiver_E001c_AnyVisiblePatch/
%     Tiles_1m/...
%
...
p.addParameter('outputBaseRoot', '/tank/data/SFS/xinyis/data/bathymetry/Processed_Results/Tiles_for_MAE_FullRiver_E001c_AnyVisiblePatch', @ischar);
...
fprintf('\n============================================================\n');
fprintf('E001c V1 NoDataSafe full-river tiles with ANY-VISIBLE strict-reverse patch HiddenMask\n');
...
fprintf('\n============================================================\n');
fprintf('E001c V1 ANY-VISIBLE finished.\n');
fprintf('============================================================\n');
end

function processOneRiver(river, cfg)
fprintf('\n============================================================\n');
fprintf('[E001c] River: %s\n', river);
fprintf('============================================================\n');
...
for ii = 1:numel(idxSpacing)
    ...
    if cfg.writeHiddenMask && hasHidden
        ...
        % SAME STRICT-REVERSE rule used by D001b training tiles:
...
qaCsv = fullfile(folders.QA, sprintf('E001c_candidate_QA_%s_%s.csv', resStr, river));
writetable(qaTable, qaCsv);
fprintf('QA CSV: %s\n', qaCsv);

if ~isempty(candidateStruct)
    outCandPts = fullfile(folders.QA, sprintf('E001c_candidate_points_QA_%s_%s.shp', resStr, river));
...
if ~isempty(selectedStruct)
    outPts = fullfile(folders.QA, sprintf('E001c_selected_points_%s_%s.shp', resStr, river));
...
if ~isempty(manifestRows)
    ...
    manifestCsv = fullfile(folders.QA, sprintf('E001c_tile_manifest_%s_%s.csv', resStr, river));
writetable(manifestTable, manifestCsv);
    fprintf('Manifest CSV: %s\n', manifestCsv);

    writeList(fullfile(folders.Lists, sprintf('E001c_fullriver_tiles_%s_%s.txt', resStr, river)), manifestTable.dem_path);
    if ismember('hidden_mask_path', manifestTable.Properties.VariableNames)
        writeList(fullfile(folders.Lists, sprintf('E001c_hidden_masks_336patchview_%s_%s.txt', resStr, river)), manifestTable.hidden_mask_path);
    end
    if ismember('loss_mask_pixel_path', manifestTable.Properties.VariableNames)
        writeList(fullfile(folders.Lists, sprintf('E001c_loss_masks_pixel_%s_%s.txt', resStr, river)), manifestTable.loss_mask_pixel_path);
    end
    if ismember('core_loss_mask_pixel_path', manifestTable.Properties.VariableNames)
        writeList(fullfile(folders.Lists, sprintf('E001c_core_loss_masks_pixel_%s_%s.txt', resStr, river)), manifestTable.core_loss_mask_pixel_path);
    end
end
end
