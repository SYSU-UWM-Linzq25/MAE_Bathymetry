function D001c_AnyVisiblePatch(rivers, varargin)
% D001c_AnyVisiblePatch
%
% Training-tile extraction derived directly from:
%   D001_SelectTiles_Overlap_DualMask_ForRiver_V10_NoDataSafe
%
% Teacher-agreed patch HiddenMask rule: STRICT REVERSE / ANY-VISIBLE.
%
% Pixel definitions:
%   hiddenPixel  = valid pixel marked by the source HiddenMask as hidden/bathy.
%   visiblePixel = valid DEM pixel that is NOT marked hidden.
%
% Patch conversion for each 16 x 16 model patch:
%   visiblePatchByRule = ANY visiblePixel in the patch.
%   hiddenPatch        = NOT visiblePatchByRule.
%
% Therefore:
%   - if a patch contains at least ONE valid visible pixel, the ENTIRE patch
%     is supplied to the model as visible;
%   - a patch is hidden only when it contains ZERO valid visible pixels;
%   - for a fully valid patch this is equivalent to:
%         hiddenPatch = ALL(hiddenPixel).
%
% This is the exact logical inverse of the original strict D001 rule:
%   original: one hidden pixel  -> whole patch hidden;
%   new:      one visible pixel -> whole patch visible.
%
% Important scientific consequence:
%   A mixed patch can expose bathymetry/hidden pixels together with its one or
%   more visible pixels. This is intentional under the new teacher-agreed rule.
%
% All other D001 V10 logic is retained:
%   - centerline thinning and spacing
%   - 336 x 336 tiles and 16 x 16 patches
%   - merged DEM and canonical NoData handling
%   - bathy-valid-gated pixel LossMask
%   - core region and training-tile selection thresholds
%   - QA points, manifests and paired lists
%
% Loss behavior:
%   Hidden_Mask controls model input visibility. Loss_Mask_Pixel remains the
%   exact bathymetry supervision mask. With the existing exact-LCC/core-loss
%   training backend, only prediction/hidden patches contribute to patch loss;
%   newly visible patches are input context rather than reconstruction targets.
%
% Training-tile safeguard:
%   minEffectiveCoreLossPixelCount ensures every retained tile still has at
%   least one core loss pixel inside a patch that remains hidden. This avoids
%   zero-supervision tiles after the ANY-VISIBLE rule opens mixed patches.
%
% Output:
%   Processed_Results/Tiles_for_MAE_v2_D001c_AnyVisiblePatch/
%     Tiles_1m/...
%
% Output filenames keep the original D001 naming patterns so existing training
% readers can reuse them by changing only the dataset root.
%
% Example:
%   D001c_AnyVisiblePatch( ...
...
p.addParameter('outputBaseRoot', '/tank/data/SFS/xinyis/data/bathymetry/Processed_Results/Tiles_for_MAE_v2_D001c_AnyVisiblePatch', @ischar);
...
p.addParameter('minEffectiveCoreLossPixelCount', 1, @(x)isnumeric(x)&&isscalar(x)&&x>=0);
...
cfg = p.Results;
cfg.ruleTag = 'AnyVisiblePatch_StrictReverse';
cfg.outputRoot = cfg.outputBaseRoot;
...
cfg.minEffectiveCoreLossPixelCount = round(cfg.minEffectiveCoreLossPixelCount);
...
fprintf('\n============================================================\n');
fprintf('D001c V1 NoDataSafe training tiles with ANY-VISIBLE strict-reverse patch HiddenMask\n');
...
fprintf('Hidden rule    : if ANY valid visible pixel exists, the whole patch is visible; otherwise hidden\n');
fprintf('Equivalent     : for a fully valid patch, hiddenPatch = ALL(hiddenPixel)\n');
fprintf('Hidden output  : 336x336 ANY-VISIBLE patch-view mask for model\n');
...
fprintf('Effective hidden core-loss pixel min: count >= %d\n', cfg.minEffectiveCoreLossPixelCount);
...
fprintf('\n============================================================\n');
fprintf('D001c V1 ANY-VISIBLE finished.\n');
fprintf('============================================================\n');
end
...
for ii = 1:numel(idxSpacing)
    ...
    if r1 < 1 || c1 < 1 || (r1 + h - 1) > rowsM || (c1 + w - 1) > colsM
        ...
        qaRows = appendQARow(qaRows, srcIdx, NaN, x, y, lineID(srcIdx), width(srcIdx), ...
            row0, col0, false, reject, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN);
    ...
    % STRICT-REVERSE rule agreed with the advisor:
    % one valid visible pixel makes the entire 16x16 patch visible.
    visiblePixel = (~hiddenPixel) & inputValid;
    visiblePatchByRule = blockAny(visiblePixel, cfg.patchSize);
    hiddenPatch = ~visiblePatchByRule;
    ...
    hiddenPatchPixelView = logical(expandPatchMask(hiddenPatch, cfg.patchSize));
    effectiveCoreLossPixel = finalLossPixel & corePixelMask & hiddenPatchPixelView;
    effectiveCoreLossPixelCount = nnz(effectiveCoreLossPixel);
    ...
    elseif effectiveCoreLossPixelCount < cfg.minEffectiveCoreLossPixelCount
        reject = "effective_core_loss_pixel_count";
    ...
    if isKeep
        ...
        if selectedCount <= cfg.maxTilesPerRiver
            ...
            selectedStruct = appendPointStruct(selectedStruct, pointID, srcIdx, x, y, lineID(srcIdx), width(srcIdx), ...
                coreLossCount, coreLossRatio, coreLossPixelCount, coreLossPixelRatio, coreHiddenCount, hiddenPatchCount, lossPatchCount, visiblePatchCount, knownPatchRatio, effectiveCoreLossPixelCount);
        end
    end
    ...
    qaRows = appendQARow(qaRows, srcIdx, pointID, x, y, lineID(srcIdx), width(srcIdx), ...
        ...
        hiddenPatchCount, lossPatchCount, visiblePatchCount, knownPatchCount, knownPatchRatio, coreHiddenCount, coreLossCount, coreLossRatio, coreLossPixelCount, coreLossPixelRatio, effectiveCoreLossPixelCount);
...
qaCsv = fullfile(folders.QA, sprintf('D001c_candidate_patch_QA_%s_%s.csv', resStr, river));
writetable(qaTable, qaCsv);
fprintf('QA CSV: %s\n', qaCsv);

% Write all spacing-thinned candidate points with keep/reject QA fields.
if ~isempty(candidateStruct)
    outCandPts = fullfile(folders.QA, sprintf('D001c_candidate_points_QA_%s_%s.shp', resStr, river));
...
if ~isempty(selectedStruct)
    outPts = fullfile(folders.QA, sprintf('D001c_selected_points_%s_%s.shp', resStr, river));
...
if ~isempty(manifestRows)
    ...
    manifestCsv = fullfile(folders.QA, sprintf('D001c_tile_manifest_%s_%s.csv', resStr, river));
    writetable(manifestTable, manifestCsv);
    fprintf('Manifest CSV: %s\n', manifestCsv);

    writeList(fullfile(folders.Lists, sprintf('D001c_train_tiles_%s_%s.txt', resStr, river)), manifestTable.dem_path);
    writeList(fullfile(folders.Lists, sprintf('D001c_hidden_masks_336patchview_%s_%s.txt', resStr, river)), manifestTable.hidden_mask_path);
    writeList(fullfile(folders.Lists, sprintf('D001c_loss_pixel_masks_%s_%s.txt', resStr, river)), manifestTable.loss_pixel_mask_path);
    writeList(fullfile(folders.Lists, sprintf('D001c_hidden_patch21_QA_%s_%s.txt', resStr, river)), manifestTable.hidden_patch21_QA_path);
    writeList(fullfile(folders.Lists, sprintf('D001c_loss_patchview_QA_%s_%s.txt', resStr, river)), manifestTable.loss_patchview_QA_path);
    writeList(fullfile(folders.Lists, sprintf('D001c_loss_patch21_QA_%s_%s.txt', resStr, river)), manifestTable.loss_patch21_QA_path);
end
end
...
function rows = appendQARow(rows, srcIdx, pointID, x, y, lineID, width, row0, col0, kept, reject, ...
    hiddenPixRatio, lossPixRatio, bathyValidRatio, hiddenPatchCount, lossPatchCount, visiblePatchCount, knownPatchCount, knownPatchRatio, coreHiddenCount, coreLossCount, coreLossRatio, coreLossPixelCount, coreLossPixelRatio, effectiveCoreLossPixelCount)
r.src_idx = srcIdx;
r.point_id = pointID;
r.x = x;
r.y = y;
...
r.effective_core_loss_pixel_count = effectiveCoreLossPixelCount;
if isempty(rows)
    rows = r;
else
    rows(end+1,1) = r;
...
r.EffCLPixN = safeNum(effectiveCoreLossPixelCount, -9999);
if isempty(S)
    S = r;
else
    S(end+1,1) = r;
...
switch s
    ...
    case 'visible_patch_count'
        c = 9;
    case 'effective_core_loss_pixel_count'
        c = 10;
    otherwise
        c = 99;
end
...
function S = appendPointStruct(S, pointID, srcIdx, x, y, lineID, width, coreLoss, coreLossRatio, coreLossPix, coreLossPixRatio, coreHidden, hiddenPatch, lossPatch, visiblePatch, knownPatchRatio, effectiveCoreLossPixelCount)
...
r.EffCLPixN = effectiveCoreLossPixelCount;
if isempty(S)
    S = r;
else
    S(end+1,1) = r;
