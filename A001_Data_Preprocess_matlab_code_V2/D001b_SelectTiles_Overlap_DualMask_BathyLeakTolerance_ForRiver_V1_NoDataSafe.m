function D001b_SelectTiles_Overlap_DualMask_BathyLeakTolerance_ForRiver_V1_NoDataSafe(rivers, varargin)
% D001b_SelectTiles_Overlap_DualMask_BathyLeakTolerance_ForRiver_V1_NoDataSafe
%
...
%
% Original strict D001 rule:
%   hidden_patch = ANY hidden/bathymetry pixel in the patch.
%   Therefore one hidden pixel masks the entire 16 x 16 patch.
%
% D001b bathymetry-leakage-tolerance rule:
...
%       hidden_count >  floor(maxBathyLeakRatio * patchSize^2)
%
% Examples for patchSize=16 (256 pixels):
%   maxBathyLeakRatio=0.00 -> allow 0 hidden pixels  (original strict rule)
%   maxBathyLeakRatio=0.05 -> allow up to 12 hidden pixels
%   maxBathyLeakRatio=0.10 -> allow up to 25 hidden pixels
%   maxBathyLeakRatio=0.25 -> allow up to 64 hidden pixels
%   maxBathyLeakRatio=0.50 -> allow up to 128 hidden pixels
%
...
%   - core region and tile-selection thresholds
%   - QA points, manifests, and paired lists
%
% Important:
%   The relaxed HiddenMask is used both as the model mask and when computing
%   visible/known patch counts for training-tile selection. The original
%   strict D001 dataset already exists and is not recreated here.
%
% Output isolation:
%   Processed_Results/Tiles_for_MAE_v2_D001b_BathyLeakTolerance/
%     BathyLeak025pct/Tiles_1m/...
%
% Output filenames keep the original D001 naming patterns so existing
% training readers can reuse them by changing only the dataset root.
%
% Example:
%   D001b_SelectTiles_Overlap_DualMask_BathyLeakTolerance_ForRiver_V1_NoDataSafe( ...
...
p.addParameter('outputBaseRoot', '/tank/data/SFS/xinyis/data/bathymetry/Processed_Results/Tiles_for_MAE_v2_D001b_BathyLeakTolerance', @ischar);
...
fprintf('\n============================================================\n');
fprintf('D001b V1 NoDataSafe training tiles with bathymetry-leakage-tolerant patch HiddenMask\n');
...
fprintf('Hidden output  : 336x336 leakage-tolerant patch-view mask for model\n');
...
qaCsv = fullfile(folders.QA, sprintf('D001b_candidate_patch_QA_%s_%s.csv', resStr, river));
writetable(qaTable, qaCsv);
fprintf('QA CSV: %s\n', qaCsv);

% Write all spacing-thinned candidate points with keep/reject QA fields.
if ~isempty(candidateStruct)
    outCandPts = fullfile(folders.QA, sprintf('D001b_candidate_points_QA_%s_%s.shp', resStr, river));
...
if ~isempty(selectedStruct)
    outPts = fullfile(folders.QA, sprintf('D001b_selected_points_%s_%s.shp', resStr, river));
...
if ~isempty(manifestRows)
    ...
    manifestCsv = fullfile(folders.QA, sprintf('D001b_tile_manifest_%s_%s.csv', resStr, river));
    writetable(manifestTable, manifestCsv);
    fprintf('Manifest CSV: %s\n', manifestCsv);

    writeList(fullfile(folders.Lists, sprintf('D001b_train_tiles_%s_%s.txt', resStr, river)), manifestTable.dem_path);
    writeList(fullfile(folders.Lists, sprintf('D001b_hidden_masks_336patchview_%s_%s.txt', resStr, river)), manifestTable.hidden_mask_path);
    writeList(fullfile(folders.Lists, sprintf('D001b_loss_pixel_masks_%s_%s.txt', resStr, river)), manifestTable.loss_pixel_mask_path);
    writeList(fullfile(folders.Lists, sprintf('D001b_hidden_patch21_QA_%s_%s.txt', resStr, river)), manifestTable.hidden_patch21_QA_path);
    writeList(fullfile(folders.Lists, sprintf('D001b_loss_patchview_QA_%s_%s.txt', resStr, river)), manifestTable.loss_patchview_QA_path);
    writeList(fullfile(folders.Lists, sprintf('D001b_loss_patch21_QA_%s_%s.txt', resStr, river)), manifestTable.loss_patch21_QA_path);
end
end
