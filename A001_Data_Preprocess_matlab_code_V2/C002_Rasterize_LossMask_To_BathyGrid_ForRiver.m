function Summary = C002_Rasterize_LossMask_To_BathyGrid_ForRiver(riverName, targetResM, varargin)
% C002_Rasterize_LossMask_To_BathyGrid_ForRiver
% 2026-07-05
%
% Purpose
%   Rasterize manually edited LossMask.shp to the bathymetry grids at
%   1 m / 3 m / 5 m / 10 m resolutions.
%
% Input shapefile
%   /tank/data/SFS/xinyis/data/bathymetry/LossMask_Draw/Mask_ShapeFiles/<river>/LossMask.shp
%
% Reference grids
%   /tank/data/SFS/xinyis/data/bathymetry/Processed_Results/Bathy_<res>m_FixND/<river>/Bathy_<res>m.vrt
%
% Output
%   /tank/data/SFS/xinyis/data/bathymetry/LossMask_Draw/LossMask_ByRiver_<res>m/<river>/LossMask_<res>m.tif
%   /tank/data/SFS/xinyis/data/bathymetry/LossMask_Draw/LossMask_ByRiver_<res>m/<river>/LossMask_<res>m.vrt
%
% Mask values
%   1   = loss-valid area, burn from LossMask.shp
%   0   = non-loss background inside the reference grid extent
...
%   - Default uses -at, so any pixel touched by the polygon becomes loss=1.
...
%
% Usage examples
%   C002_Rasterize_LossMask_To_BathyGrid_ForRiver('MD_PotomacRiver_Bathy_2019');
%   C002_Rasterize_LossMask_To_BathyGrid_ForRiver('CA_KlamathRiver_TopoBathy_2018_D18', [1 3 5 10]);
%   C002_Rasterize_LossMask_To_BathyGrid_ForRiver('OR_MKRC_Topobathy_2021', [3 5 10]);
%   C002_Rasterize_LossMask_To_BathyGrid_ForRiver({'MD_PotomacRiver_Bathy_2019','CA_KlamathRiver_TopoBathy_2018_D18'}, [10]);
%   C002_Rasterize_LossMask_To_BathyGrid_ForRiver('ALL');
%   C002_Rasterize_LossMask_To_BathyGrid_ForRiver('LIST');
...
fprintf('\n============================================================\n');
fprintf('C002 LossMask shapefile -> bathy-grid raster\n');
fprintf('Loss SHP root : %s\n', cfg.lossShpRoot);
fprintf('Bathy root    : %s\n', cfg.prRoot);
fprintf('Output root   : %s/LossMask_ByRiver_<res>m\n', cfg.lossRoot);
fprintf('Rivers        : %s\n', strjoin(riversToRun, ', '));
...
for r = 1:numel(riversToRun)
    river = riversToRun{r};
    shp = fullfile(cfg.lossShpRoot, river, 'LossMask.shp');
    ...
    if exist(shp, 'file') ~= 2
        msg = sprintf('Missing LossMask.shp: %s', shp);
    ...
    for j = 1:numel(targetResM)
        ...
        outRoot = fullfile(cfg.lossRoot, sprintf('LossMask_ByRiver_%dm', resM), river);
        ensureDir(outRoot);

        outTif = fullfile(outRoot, sprintf('LossMask_%dm.tif', resM));
        outVrt = fullfile(outRoot, sprintf('LossMask_%dm.vrt', resM));
        infoJson = fullfile(outRoot, 'reference_grid_gdalinfo.json');
        srsWkt = fullfile(outRoot, 'target_bathy_srs.wkt');

        fprintf('\n============================================================\n');
        fprintf('[%s] Rasterize LossMask to %dm bathy grid\n', river, resM);
        fprintf('Reference: %s\n', refGrid);
        fprintf('Output   : %s\n', outTif);
        fprintf('============================================================\n');
        ...
        try
            ...
            if strcmpi(modeUsed, 'tiled')
                outMain = rasterizeLossMaskTiled(shpUse, srsWkt, info, outRoot, resM, cfg);
                outVrt = outMain;
                outTif = '';
                statusText = 'OK_TILED_VRT';
            else
                rasterizeLossMaskSingle(shpUse, srsWkt, info, outTif, cfg);
...
summaryFile = fullfile(cfg.logRoot, sprintf('C002_LossMask_Rasterize_%s_%s.csv', safeName(strjoin(riversToRun, '__')), datestr(now, 'yyyymmdd_HHMMSS')));
writeSummaryCSV(summaryFile, summaryRows);

fprintf('\n============================================================\n');
fprintf('C002 LossMask rasterization finished. Summary written:\n%s\n', summaryFile);
fprintf('Outputs should be under:\n%s/LossMask_ByRiver_<res>m/<river>/\n', cfg.lossRoot);
fprintf('============================================================\n');
...
function cfg = defaultConfig()
    ...
    cfg.lossRoot = '/tank/data/SFS/xinyis/data/bathymetry/LossMask_Draw';
    cfg.lossShpRoot = fullfile(cfg.lossRoot, 'Mask_ShapeFiles');
    cfg.logRoot = fullfile(cfg.lossRoot, 'Logs');
    ...
    % outputMode:
    %   'single' = write one LossMask_<res>m.tif
    %   'tiled'  = write LossMask_<res>m_tiles/*.tif + LossMask_<res>m.vrt
...
function cfg = parseOptions(cfg, varargin)
    ...
    for i = 1:2:numel(varargin)
        ...
        switch key
            case 'processedroot'
                cfg.prRoot = char(val);
            case {'lossroot','maskroot'}
                cfg.lossRoot = char(val);
                cfg.lossShpRoot = fullfile(cfg.lossRoot, 'Mask_ShapeFiles');
                cfg.logRoot = fullfile(cfg.lossRoot, 'Logs');
            case {'lossshproot','maskshproot'}
                cfg.lossShpRoot = char(val);
            case 'outputmode'
...
function printRiverList(rivers)
    fprintf('\nSelected valid rivers for C002 LossMask rasterization:\n');
...
function shpUse = prepareShapefileForTargetSRS(shp, srsWkt, outRoot, cfg)
    ...
    reprojDir = fullfile(outRoot, '_LossMask_reproj_to_bathy_srs');
    ensureDir(reprojDir);
    shpUse = fullfile(reprojDir, 'LossMask_reproj.shp');
    ...
    if status ~= 0 || exist(shpUse, 'file') ~= 2
        error('ogr2ogr failed to reproject LossMask.shp: %s', msg);
    end
end

function rasterizeLossMaskSingle(shpUse, srsWkt, info, outTif, cfg)
...
function outVrt = rasterizeLossMaskTiled(shpUse, srsWkt, info, outRoot, resM, cfg)
    tileDir = fullfile(outRoot, sprintf('LossMask_%dm_tiles', resM));
    ensureDir(tileDir);

    outVrt = fullfile(outRoot, sprintf('LossMask_%dm.vrt', resM));
    tileList = fullfile(tileDir, sprintf('LossMask_%dm_tile_list.txt', resM));

    if cfg.overwrite
        oldTiles = dir(fullfile(tileDir, sprintf('LossMask_%dm_tile_*.tif', resM)));
    ...
    for r0 = 0:cfg.tileSize:(info.rows-1)
        ...
        for c0 = 0:cfg.tileSize:(info.cols-1)
            ...
            tileName = sprintf('LossMask_%dm_tile_r%06d_c%06d.tif', resM, r0, c0);
...
function quickMaskStats(rasterPath, cfg)
    ...
    system(cmd);
    fprintf('Expected value convention: loss=1, background=0, nodata(meta)=%d\n', cfg.maskNoData);
end
...
function T = rowsToTable(rows)
    ...
    T = cell2table(rows, 'VariableNames', {'river','res_m','refGrid','lossShp','outTif','outVrt','status','message'});
end
...
function writeSummaryCSV(summaryFile, rows)
    ...
    fprintf(fid, 'river,res_m,refGrid,lossShp,outTif,outVrt,status,message\n');
    for i = 1:size(rows,1)
