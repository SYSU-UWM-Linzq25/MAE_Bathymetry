function D001_SelectTiles_Overlap_DualMask_ForRiver_V3(rivers, varargin)
% D001_SelectTiles_Overlap_DualMask_ForRiver_V3
%
...
%   D001_SelectTiles_Overlap_DualMask_ForRiver_V3('Kletzch_Combined_UpMax3Null');
%   D001_SelectTiles_Overlap_DualMask_ForRiver_V3('ALL');
%
% Useful options:
...
fprintf('\n============================================================\n');
fprintf('D001 V2 Select MAE tiles with overlap + dual masks\n');
        ...
        if selectedCount <= cfg.maxTilesPerRiver
            if ~cfg.skipWriteTiles
                ...
                if cfg.writeHiddenPixelQA
                    writeMaybe(hidPixOut, uint8(hiddenPixel), subGT, proj, 1, 'GTiff', 255, cfg.overwrite);
                else
                    hidPixOut = '';
                end
                ...
                writeMaybe(losPixOut, uint8(finalLossPixel), subGT, proj, 1, 'GTiff', 255, cfg.overwrite);
                ...
                if cfg.writePatch21
                    writeMaybe(hid21Out, uint8(hiddenPatch), patchGT, proj, 1, 'GTiff', 255, cfg.overwrite);
                    writeMaybe(los21Out, uint8(lossPatch), patchGT, proj, 1, 'GTiff', 255, cfg.overwrite);
                else
                    hid21Out = '';
                    los21Out = '';
                end
...
out = uint8(kron(uint8(P), ones(p,p,'uint8')));
end
...
if exist(outPath, 'file') == 2 && ~overwrite
    return;
end
