...
# 任务2B：修改这里
# =========================
ROOT = Path(r"/tank/data/SFS/xinyis/data/bathymetry/MAE-Topography/Upstream_Model_ReTrain/eval_KY_task2B_compare")
OUTDIR = ROOT / "plots_task2b_compare"
...
RUNS = {
    ...
    "stage2B_KY_decoder_mr_0.20": "DEC-0.20",
}

# 重点 baseline：同样是 0.20，比较 decoder 和 upstream
BASELINE_KEY = "upstream_mr_0.20"
DECODER_KEY = "stage2B_KY_decoder_mr_0.20"


def load_all_results(root: Path, runs: dict[str, str]):
    summaries = {}
    metrics = {}
...
def plot_upstream_trend(summary_df: pd.DataFrame, out_png: Path):
    ...
    plt.title("KY val: upstream rmse_m_mask vs mask_ratio")
...
def plot_all_boxplot(metrics: dict, runs: dict[str, str], out_png: Path):
    ...
    plt.ylabel("rmse_m_mask (m)")
    plt.title("KY val: rmse_m_mask distribution")
...
def plot_decoder_delta_hist(delta_df: pd.DataFrame, out_png: Path):
    ...
    plt.ylabel("count")
    plt.title("KY val: decoder-20 vs upstream-20 paired delta")
    plt.grid(True, axis="y", alpha=0.3)
    plt.tight_layout()
    plt.savefig(out_png, dpi=200)
    plt.close()
...
def main():
    ...
    summary_csv = OUTDIR / "task2b_rmse_m_mask_summary_table.csv"
    summary_df.to_csv(summary_csv, index=False)

    plot_upstream_trend(summary_df, OUTDIR / "task2b_upstream_trend.png")
    plot_all_boxplot(metrics, RUNS, OUTDIR / "task2b_all_boxplot.png")

    delta_df = build_decoder_vs_up20_delta(metrics)
    delta_csv = OUTDIR / "task2b_decoder_vs_up20_delta.csv"
    delta_df.to_csv(delta_csv, index=False)

    plot_decoder_delta_hist(delta_df, OUTDIR / "task2b_decoder_vs_up20_delta_hist.png")
    plot_decoder_delta_box(delta_df, OUTDIR / "task2b_decoder_vs_up20_delta_box.png")

    print("[DONE]", summary_csv)
    print("[DONE]", OUTDIR / "task2b_upstream_trend.png")
    print("[DONE]", OUTDIR / "task2b_all_boxplot.png")
    print("[DONE]", delta_csv)
    print("[DONE]", OUTDIR / "task2b_decoder_vs_up20_delta_hist.png")
    print("[DONE]", OUTDIR / "task2b_decoder_vs_up20_delta_box.png")
