#!/usr/bin/env python3
# F046_relax_fullriver_gt_error_MeterOnly_UniquePixel.py
# D001c/E001c AnyVisiblePatch RELAX branch.
# Exact unique-geospatial-pixel metric logic is preserved from F032/F046.
"""F046: build GT/error GeoTIFFs and exact unique-pixel metrics for F044 D001c/E001c MeterOnly predictions.

Inputs
------
Per-river F044 TileAvgVRT outputs:
...
For each averaged prediction tile, this script reads:
  ...
  - F044 averaged prediction tile

Outputs
-------
Per river:
  gt_core_final_loss_tile/
    F046_gt_m_<key>_core_final_loss.tif
  error_signed_m_tile/
    F046_err_signed_m_<key>_core_final_loss.tif
  error_abs_m_tile/
    F046_err_abs_m_<key>_core_final_loss.tif

  F046_fullriver_gt_m_<river>_core_final_loss_tiles.vrt
  F046_fullriver_err_signed_m_<river>_core_final_loss_tiles.vrt
  F046_fullriver_err_abs_m_<river>_core_final_loss_tiles.vrt
  F046_tile_error_metrics.csv
  F046_summary.json
...
The corrected F044 writes valid_prediction only inside the model-returned
prediction_patch_mask. Therefore newly-visible E001c patches cannot enter
F046 metrics. F046 refuses older F044 outputs without the correction marker.
"""
...
def find_river_dirs(f044_out_dir: Path, rivers: Sequence[str]) -> List[Path]:
    f044_out_dir = Path(f044_out_dir)
    if rivers:
        dirs = [f044_out_dir / r for r in rivers]
    else:
        dirs = [p for p in sorted(f044_out_dir.iterdir()) if p.is_dir() and (p / 'F044_summary.json').exists()]
    missing = [str(d) for d in dirs if not (d / 'F044_summary.json').exists()]
    if missing:
        raise FileNotFoundError('Missing F044_summary.json for river dirs:\n' + '\n'.join(missing))
    return dirs
...
def process_river(river_dir: Path, out_base: Path, args) -> Dict[str, Any]:
    ...
    summary_path = river_dir / 'F044_summary.json'
    manifest_path = river_dir / 'F044_tileavg_prediction_manifest.csv'
    if not manifest_path.exists():
        raise FileNotFoundError(f'Missing F044 manifest: {manifest_path}')

    f044_summary = json.loads(summary_path.read_text())
    if f044_summary.get('prediction_patch_filter_applied') is not True:
        raise RuntimeError(
            'Refusing leakage-prone/legacy F044 output without '
    ...
    for idx, r in enumerate(rows, start=1):
        ...
        gt_out_path = gt_dir / f'F046_gt_m_{key}_core_final_loss.tif'
        err_signed_path = err_signed_dir / f'F046_err_signed_m_{key}_core_final_loss.tif'
        err_abs_path = err_abs_dir / f'F046_err_abs_m_{key}_core_final_loss.tif'
    ...
    if not metric_rows:
        raise RuntimeError(f'No F046 outputs produced for river={river}.')

    gt_vrt = out_river / f'F046_fullriver_gt_m_{river}_core_final_loss_tiles.vrt'
    err_signed_vrt = out_river / f'F046_fullriver_err_signed_m_{river}_core_final_loss_tiles.vrt'
    err_abs_vrt = out_river / f'F046_fullriver_err_abs_m_{river}_core_final_loss_tiles.vrt'
    ...
    write_csv(out_river / 'F046_tile_error_metrics.csv', metric_rows)
    write_csv(out_river / 'F046_skipped_tiles.csv', skipped)
    ...
    summary = {
        ...
        'comparison_mask': ('Core_Loss_Mask_Pixel AND valid_GT AND valid_prediction; ''F044 valid_prediction is restricted to model prediction patches'),
    ...
    (out_river / 'F046_summary.json').write_text(json.dumps(summary, indent=2))
...
def main() -> None:
    ...
    ap.add_argument(
        '--f044_out_dir', '--f060_out_dir',
        dest='f044_out_dir',
        required=True,
        help='F044 full-river prediction output folder. --f060_out_dir is retained as a legacy alias.',
    )
    ...
    f044_out = Path(args.f044_out_dir)
    out = Path(args.output_dir)
    out.mkdir(parents=True, exist_ok=True)
    river_dirs = find_river_dirs(f044_out, args.rivers)

    (out / 'F046_args.json').write_text(json.dumps({
        **vars(args),
        'resolved_river_dirs': [str(d) for d in river_dirs],
    }, indent=2))

    print('[F046] Build GT/error VRTs plus legacy tile-footprint and exact unique-pixel metrics')
    print(f'[F046] f044_out_dir={f044_out}')
    print(f'[F046] output_dir={out}')
    print(f'[F046] rivers={[d.name for d in river_dirs]}')

    summaries = []
    for d in river_dirs:
        summaries.append(process_river(d, out, args))

    (out / 'F046_all_rivers_summary.json').write_text(json.dumps(summaries, indent=2))
    print('[DONE]', out)
