...
# D044: MAE v2 dual-mask meter-MAE backend with pre-training baseline evaluation.
#
# Inputs:
...
RUN_NAME=${RUN_NAME:-D044_meterOnly_D001c_AnyVisiblePatch_e400}
...
if [[ "$OPTIMIZATION_LOSS" != "meter_mae" ]]; then
  echo "[ERROR] MeterOnly requires OPTIMIZATION_LOSS=meter_mae; got $OPTIMIZATION_LOSS" >&2
  exit 2
fi
if [[ "$BEST_METRIC" != "val_mae_m_mask" || "$EARLY_STOP_METRIC" != "val_mae_m_mask" ]]; then
  echo "[ERROR] MeterOnly requires BEST_METRIC=val_mae_m_mask and EARLY_STOP_METRIC=val_mae_m_mask" >&2
  exit 2
fi
...
date
echo "=== D044 MAE v2 dual-mask meter-MAE + baseline-eval training done ==="
echo "RUN=$OUT_DIR"
