...
# D048: Stage-2 dual-mask fine-tuning backend.
#
# Stage 1 (already completed in cross_validation_v2):
#   optimization objective = normalized-space MSE
#   selected checkpoint     = minimum normalized validation loss (val_loss)
#
# Stage 2 (this script):
#   initialization          = Stage-1 normalized-loss checkpoint-best.pth
#   optimization objective  = exact meter-space MAE on the core loss pixels
#   selected checkpoint     = minimum validation meter MAE
#
# The untouched Stage-1 normalized model is evaluated as epoch -1 and copied to
# checkpoint-best.pth before any Stage-2 optimizer update. Stage 2 therefore
# cannot replace the formal best model unless validation meter MAE improves.
...
STAGE1_NORM_CKPT=${STAGE1_NORM_CKPT:?Please set STAGE1_NORM_CKPT to the normalized-loss checkpoint-best.pth}

RUN_NAME=${RUN_NAME:-D048_normThenMeter_D001c_AnyVisiblePatch}
...
# Formal frozen-encoder baseline uses 0. Later partial-unfreeze runs may set 1.
TRAINABLE_LAST_N_ENCODER_BLOCKS=${TRAINABLE_LAST_N_ENCODER_BLOCKS:-0}
...
if [[ "$OPTIMIZATION_LOSS" != "meter_mae" ]]; then
  echo "[ERROR] NormThenMeter requires OPTIMIZATION_LOSS=meter_mae; got $OPTIMIZATION_LOSS" >&2
  exit 2
fi
if [[ "$BEST_METRIC" != "val_mae_m_mask" || "$EARLY_STOP_METRIC" != "val_mae_m_mask" ]]; then
  echo "[ERROR] NormThenMeter requires BEST_METRIC=val_mae_m_mask and EARLY_STOP_METRIC=val_mae_m_mask" >&2
  exit 2
fi
...
if [[ -d "$OUT_DIR" ]] && find "$OUT_DIR" -mindepth 1 -print -quit | grep -q .; then
  if [[ "$OVERWRITE_STAGE2" == "1" ]]; then
    ...
    echo "Use FRESH_RUN=0 to resume, choose a new RUN_NAME, or set OVERWRITE_STAGE2=1." >&2
    exit 4
  fi
fi
...
echo "============================================================"
echo "D048 Stage-2 meter-MAE optimization from normalized-loss checkpoint"
date
echo "HOST=$(hostname)"
echo "UNIFIED_TRAINER=$CODE/main_pretrain_dem_unified_relax.py"
echo "UNIFIED_ENGINE=$CODE/engine_pretrain_unified_relax.py"
echo "FORMAL_ROUTE=NormThenMeter"
...
echo "BASELINE_EPOCH=-1 (untouched Stage-1 normalized-loss checkpoint)"
echo "STAGE2_LR=$LR"
echo "MIN_LR=$MIN_LR"
...
echo "SAFETY=checkpoint-best remains normalized Stage 1 unless Stage 2 improves validation meter MAE"
echo "============================================================"

printf '%s\n' "$STAGE1_NORM_CKPT" > "$OUT_DIR/stage1_normalized_source_checkpoint.txt"
sha256sum "$STAGE1_NORM_CKPT" | tee "$OUT_DIR/stage1_normalized_source_checkpoint_sha256.txt"

STAGE1_NORM_DIR=$(dirname "$STAGE1_NORM_CKPT")
for candidate in log.txt args.json best_summary.json baseline_summary.json input_split_sha256.txt; do
  if [[ -f "$STAGE1_NORM_DIR/$candidate" ]]; then
    cp -pf "$STAGE1_NORM_DIR/$candidate" "$OUT_DIR/stage1_normalized_${candidate}"
  fi
done
...
for key in \
  ...
  if ! grep -q -- "$key" "$OUT_DIR/help.txt"; then
    echo "[ERROR] meter-MAE main script lacks expected argument: $key" >&2
    exit 3
  fi
done
...
if [[ "$FRESH_RUN" != "1" ]]; then
  ...
  if [[ -n "${CKPT:-}" ]]; then
    ...
    echo "[RESUME] No Stage-2 epoch checkpoint found; starting from normalized Stage 1."
  fi
else
  echo "[RESUME] FRESH_RUN=1; start a new Stage-2 run from normalized Stage 1."
fi
...
PYTHONUNBUFFERED=1 python -u main_pretrain_dem_unified_relax.py \
  --data_root "$TILE_ROOT" \
...
echo "============================================================"
echo "DONE D048 Stage 2 normalized -> meter"
echo "RUN=$OUT_DIR"
date
echo "============================================================"
