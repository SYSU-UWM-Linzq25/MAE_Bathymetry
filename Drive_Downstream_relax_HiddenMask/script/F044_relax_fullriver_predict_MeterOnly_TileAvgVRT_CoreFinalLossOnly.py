#!/usr/bin/env python3
# F044_relax_fullriver_predict_MeterOnly_TileAvgVRT_CoreFinalLossOnly.py
...
"""F044 full-river inference using the D001c AnyVisiblePatch MeterOnly checkpoint and E001c full-river tiles.
...
def infer_river(model, items: Sequence[TileItem], river: str, args, device: torch.device) -> Dict[str, Any]:
    ...
    for rec in sparse_records:
        ...
        avg_path = avg_tile_dir / f"F044_avg_pred_m_{rec.key}_core_final_loss.tif"
        cnt_path = count_tile_dir / f"F044_overlap_count_{rec.key}_core_final_loss.tif"
    ...
    pred_vrt = out_river / f"F044_fullriver_pred_m_{river}_core_final_loss_avg_tiles.vrt"
    count_vrt = out_river / f"F044_fullriver_overlap_count_{river}_core_final_loss_avg_tiles.vrt"
    ...
    write_csv(out_river / "F044_tileavg_prediction_manifest.csv", manifest_rows)
    write_csv(out_river / "F044_skipped_tiles.csv", skipped)
    ...
    (out_river / "F044_summary.json").write_text(json.dumps(summary, indent=2))
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "F044_summary.json and both VRT outputs are complete; otherwise "
    ...
    if not TIFFFILE_AVAILABLE:
        raise RuntimeError("tifffile is required for this F044 tile-average VRT workflow.")
    ...
    args_file = out / "F044_args.json"
    if not args_file.exists():
        args_file.write_text(json.dumps(args_payload, indent=2))
    (out / "F044_args_last_invocation.json").write_text(json.dumps(args_payload, indent=2))
    ...
    def completed_summary(summary_path: Path) -> Optional[Dict[str, Any]]:
        ...
        if summary.get("prediction_patch_filter_applied") is not True:
            print(
                "[RESUME] Old leakage-prone F044 summary detected; "
    ...
    for river in rivers:
        river_out = out / river
        summary_path = river_out / "F044_summary.json"
    ...
    for summary_path in sorted(out.glob("*/F044_summary.json")):
        summary = completed_summary(summary_path)
        if summary is not None:
            all_completed.append(summary)
    (out / "F044_all_rivers_summary.json").write_text(json.dumps(all_completed, indent=2))
