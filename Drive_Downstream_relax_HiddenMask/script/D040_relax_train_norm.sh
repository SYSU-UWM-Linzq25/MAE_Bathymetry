...
# D040: normalized-loss backend for D001c AnyVisiblePatch RELAX project training tiles.
#
# Inputs:
...
#       --core_patch_radius 3
#
# This script is called by D021. It can also be run directly after A016 has
# prepared a holdout split directory.
...
RUN_NAME=${RUN_NAME:-D040_normOnly_D001c_AnyVisiblePatch_e400}
...
# Correct early-stop default for tile-wise normalized training:
# optimize and select checkpoints in normalized loss space.
...
BASELINE_EVAL_BEFORE_TRAINING=${BASELINE_EVAL_BEFORE_TRAINING:-0}
...
if [[ "$OPTIMIZATION_LOSS" != "normalized_mse" ]]; then
  echo "[ERROR] NormOnly requires OPTIMIZATION_LOSS=normalized_mse; got $OPTIMIZATION_LOSS" >&2
  exit 2
fi
if [[ "$BEST_METRIC" != "val_loss" || "$EARLY_STOP_METRIC" != "val_loss" ]]; then
  echo "[ERROR] NormOnly requires BEST_METRIC=val_loss and EARLY_STOP_METRIC=val_loss" >&2
  exit 2
fi
...
for key in \
  ...
  early_stop_patience early_stop_metric best_metric eval_rmse; do
  if ! grep -q -- "$key" "$OUT_DIR/help.txt"; then
    echo "[ERROR] main_pretrain_dem_unified_relax.py does not support expected argument containing: $key" >&2
...
RESUME_ARGS=()
if [[ "$FRESH_RUN" != "1" ]]; then
  CKPT=$(find "$OUT_DIR" -maxdepth 1 -type f -name 'checkpoint-*.pth' ! -name 'checkpoint-best.pth' -printf '%T@ %p\n' 2>/dev/null | sort -nr | awk 'NR==1{print $2}' || true)
...
date
echo "=== D040 MAE v2 dual-mask training done ==="
echo "RUN=$OUT_DIR"
