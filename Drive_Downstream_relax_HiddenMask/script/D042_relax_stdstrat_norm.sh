...
# D042 D001NoDataSafe: make high/mid/low tile_std_safe river split, then train using normalized early stopping.
#
# Submit example:
#   sbatch -p HydroIntel -w execute-4006 --export=ALL,GPU_ID=1,STD_SPLIT_SEED=42 D042_v2_dualmask_stdstrat_normES_sbatch_20260708.sh
...
SPLIT_SCRIPT=${SPLIT_SCRIPT:-$RELAX_ROOT/script/A020s_relax_prepare_stdstrat_split.py}
...
exec >"$RUNTIME_LOG_DIR/D042_norm_std_${RUNTIME_JOB_ID}.out" 2>"$RUNTIME_LOG_DIR/D042_norm_std_${RUNTIME_JOB_ID}.err"
...
echo "=== D042 std-stratified NormOnly: D001c AnyVisiblePatch RELAX project ==="
