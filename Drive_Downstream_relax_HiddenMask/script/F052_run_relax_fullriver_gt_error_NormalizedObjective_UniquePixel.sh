#!/usr/bin/env bash
#SBATCH -J F052_relax_norm_gt_error
#SBATCH -p HydroIntel
...
SCRIPT=${SCRIPT:-$RELAX_ROOT/script/F051_relax_fullriver_gt_error_NormalizedObjective_UniquePixel.py}
RUN_TAG=${RUN_TAG:-D001cAnyVisiblePatch_D001NoDataSafe}

F049_BASE=${F049_BASE:-$RESULTS_ROOT/FullRiver_Predictions_F049_NormalizedObjective_D001cAnyVisiblePatch}
OUT_BASE=${OUT_BASE:-$RESULTS_ROOT/FullRiver_GT_Error_F051_NormalizedObjective_D001cAnyVisiblePatch}

HOLDOUT_PRESET=${HOLDOUT_PRESET:-CO}
RIVERS=${RIVERS:-}
OVERWRITE=${OVERWRITE:-0}
PROGRESS_EVERY=${PROGRESS_EVERY:-200}
F049_OUT_DIR=${F049_OUT_DIR:-}
OUT_DIR=${OUT_DIR:-}

RUNTIME_LOG_DIR="$OUT_BASE/logs"
mkdir -p "$RUNTIME_LOG_DIR"
RUNTIME_JOB_ID=${SLURM_JOB_ID:-local_$$}
exec >"$RUNTIME_LOG_DIR/F052_relax_normalized_gt_error_${HOLDOUT_PRESET}_${RUNTIME_JOB_ID}.out" \
     2>"$RUNTIME_LOG_DIR/F052_relax_normalized_gt_error_${HOLDOUT_PRESET}_${RUNTIME_JOB_ID}.err"
...
case "$HOLDOUT_PRESET" in
  ...
  *) echo "[ERROR] F052 formal workflow supports CA, CO, Santiam. Got $HOLDOUT_PRESET" >&2; exit 2 ;;
esac
RIVERS=${RIVERS:-$DEFAULT_RIVERS}

SAFE_PRESET=$(safe_name "$HOLDOUT_PRESET")
DEFAULT_TAG="holdout_${SAFE_PRESET}_${RUN_TAG}"
F049_OUT_DIR=${F049_OUT_DIR:-$F049_BASE/$DEFAULT_TAG}
...
[[ -d "$F049_OUT_DIR" ]] || { echo "[ERROR] Missing F049_OUT_DIR: $F049_OUT_DIR" >&2; exit 2; }
...
echo "============================================================"
echo "F052 D001c/E001c normalized-objective GT/error and unique-pixel metrics"
date
echo "JOB=${SLURM_JOB_ID:-local}"
echo "HOLDOUT_PRESET=$HOLDOUT_PRESET"
echo "F049_OUT_DIR=$F049_OUT_DIR"
echo "OUT_DIR=$OUT_DIR"
echo "RIVERS=$RIVERS"
echo "MASK=Core_Loss_Mask_Pixel AND valid_GT AND corrected valid_prediction"
echo "PREDICTION_QA=F049 must have prediction_patch_filter_applied=true"
