...
# D054 RELAX: Stage-2 MeterThenNorm backend for D001c AnyVisiblePatch.
#
# Stage 1 (D044/D045 MeterOnly):
...
RUN_NAME=${RUN_NAME:-D054_meterThenNorm_D001c_AnyVisiblePatch}
...
if [[ "$OPTIMIZATION_LOSS" != "normalized_mse" ]]; then
  echo "[ERROR] MeterThenNorm requires OPTIMIZATION_LOSS=normalized_mse; got $OPTIMIZATION_LOSS" >&2
  exit 2
fi
if [[ "$BEST_METRIC" != "val_mae_m_mask" || "$EARLY_STOP_METRIC" != "val_mae_m_mask" ]]; then
  echo "[ERROR] MeterThenNorm requires BEST_METRIC=val_mae_m_mask and EARLY_STOP_METRIC=val_mae_m_mask" >&2
  exit 2
fi
...
echo "FORMAL_ROUTE=MeterThenNorm"
...
echo "SAFETY=checkpoint-best remains D001c MeterOnly unless normalized Stage 2 improves validation meter MAE"
echo "============================================================"
