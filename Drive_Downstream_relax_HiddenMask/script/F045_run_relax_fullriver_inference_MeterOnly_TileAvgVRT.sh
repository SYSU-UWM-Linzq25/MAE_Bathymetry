#!/usr/bin/env bash
#SBATCH -J F045_relax_meter_fullriver
#SBATCH -p HydroIntel
...
# F045: corrected full-river reconstruction for D001c AnyVisiblePatch MeterOnly.
...
SCRIPT=${SCRIPT:-$RELAX_ROOT/script/F044_relax_fullriver_predict_MeterOnly_TileAvgVRT_CoreFinalLossOnly.py}
...
OUT_BASE=${OUT_BASE:-$RESULTS_ROOT/FullRiver_Predictions_F044_MeterOnly_D001cAnyVisiblePatch}
...
exec >"$RUNTIME_LOG_DIR/F045_relax_fullriver_${HOLDOUT_PRESET}_${RUNTIME_JOB_ID}.out" \
     2>"$RUNTIME_LOG_DIR/F045_relax_fullriver_${HOLDOUT_PRESET}_${RUNTIME_JOB_ID}.err"
...
case "$HOLDOUT_PRESET" in
  ...
  *) echo "[ERROR] F045 formal workflow supports CA, CO, Santiam. Got $HOLDOUT_PRESET" >&2; exit 2 ;;
esac
...
echo "============================================================"
echo "F045 D001c/E001c AnyVisiblePatch MeterOnly full-river inference"
date
...
echo "RIVERS=$RIVERS"
echo "MODEL_BRANCH=D001c AnyVisiblePatch MeterOnly"
