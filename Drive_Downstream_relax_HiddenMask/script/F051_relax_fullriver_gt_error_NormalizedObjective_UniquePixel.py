#!/usr/bin/env python3
# F051_relax_fullriver_gt_error_NormalizedObjective_UniquePixel.py
# D001c/E001c AnyVisiblePatch RELAX branch.
# Exact unique-geospatial-pixel metric logic is preserved from F032/F046.
"""F051: build GT/error GeoTIFFs and exact unique-pixel metrics for F049 D001c/E001c NormalizedObjective predictions.

Inputs
------
Per-river F049 TileAvgVRT outputs:
  - F049_tileavg_prediction_manifest.csv
  - F049_summary.json
...
For each averaged prediction tile, this script reads:
  ...
  - F049 averaged prediction tile

Outputs
-------
Per river:
  gt_core_final_loss_tile/
    F051_gt_m_<key>_core_final_loss.tif
  error_signed_m_tile/
    F051_err_signed_m_<key>_core_final_loss.tif
  error_abs_m_tile/
    F051_err_abs_m_<key>_core_final_loss.tif

  F051_fullriver_gt_m_<river>_core_final_loss_tiles.vrt
  F051_fullriver_err_signed_m_<river>_core_final_loss_tiles.vrt
  F051_fullriver_err_abs_m_<river>_core_final_loss_tiles.vrt
  F051_tile_error_metrics.csv
  F051_summary.json

Mask rule
---------
Final comparison footprint is:
    Core_Loss_Mask_Pixel AND valid_GT AND valid_prediction

The corrected F049 writes valid_prediction only inside the model-returned
prediction_patch_mask. Therefore newly-visible E001c patches cannot enter
F051 metrics. F051 refuses older F049 outputs without the correction marker.
"""
...
def find_river_dirs(f049_out_dir: Path, rivers: Sequence[str]) -> List[Path]:
    f049_out_dir = Path(f049_out_dir)
    if rivers:
        dirs = [f049_out_dir / r for r in rivers]
    else:
        dirs = [p for p in sorted(f049_out_dir.iterdir()) if p.is_dir() and (p / 'F049_summary.json').exists()]
    missing = [str(d) for d in dirs if not (d / 'F049_summary.json').exists()]
    if missing:
        raise FileNotFoundError('Missing F049_summary.json for river dirs:\n' + '\n'.join(missing))
    return dirs
...
def process_river(river_dir: Path, out_base: Path, args) -> Dict[str, Any]:
    ...
    summary_path = river_dir / 'F049_summary.json'
    manifest_path = river_dir / 'F049_tileavg_prediction_manifest.csv'
    if not manifest_path.exists():
        raise FileNotFoundError(f'Missing F049 manifest: {manifest_path}')

    f044_summary = json.loads(summary_path.read_text())
    if f044_summary.get('prediction_patch_filter_applied') is not True:
        raise RuntimeError(
            'Refusing leakage-prone/legacy F049 output without '
            f'prediction_patch_filter_applied=true: {river_dir}'
        )
    pred_vrt = Path(f044_summary.get('pred_vrt_path', ''))
    vrt_info = parse_vrt_basic(pred_vrt) if pred_vrt.exists() else {
        'width': int(f044_summary['virtual_mosaic_width']),
        'height': int(f044_summary['virtual_mosaic_height']),
        'transform': None,
        'srs': '',
    }
    ...
    for idx, r in enumerate(rows, start=1):
        ...
        gt_out_path = gt_dir / f'F051_gt_m_{key}_core_final_loss.tif'
        err_signed_path = err_signed_dir / f'F051_err_signed_m_{key}_core_final_loss.tif'
        err_abs_path = err_abs_dir / f'F051_err_abs_m_{key}_core_final_loss.tif'
    ...
    if not metric_rows:
        raise RuntimeError(f'No F051 outputs produced for river={river}.')

    gt_vrt = out_river / f'F051_fullriver_gt_m_{river}_core_final_loss_tiles.vrt'
    err_signed_vrt = out_river / f'F051_fullriver_err_signed_m_{river}_core_final_loss_tiles.vrt'
    err_abs_vrt = out_river / f'F051_fullriver_err_abs_m_{river}_core_final_loss_tiles.vrt'
    ...
    write_csv(out_river / 'F051_tile_error_metrics.csv', metric_rows)
    write_csv(out_river / 'F051_skipped_tiles.csv', skipped)
    ...
    summary = {
        'river': river,
        'source_f044_dir': str(river_dir),
        'n_manifest_tiles': len(rows),
        'n_tiles_written': len(metric_rows),
        'n_tiles_skipped': len(skipped),
        'comparison_mask': ('Core_Loss_Mask_Pixel AND valid_GT AND valid_prediction; ''F049 valid_prediction is restricted to model prediction patches'),
        'prediction_patch_filter_verified': True,
        ...
        'source_checkpoint': f044_summary.get('checkpoint'),
    ...
    (out_river / 'F051_summary.json').write_text(json.dumps(summary, indent=2))
...
def main() -> None:
    ...
    ap.add_argument(
        '--f049_out_dir', '--f044_out_dir', '--f060_out_dir',
        dest='f049_out_dir',
        required=True,
        help=('F049 normalized-objective full-river prediction output folder. '
              '--f044_out_dir and --f060_out_dir are retained as legacy aliases.'),
    )
    ...
    f049_out = Path(args.f049_out_dir)
    out = Path(args.output_dir)
    out.mkdir(parents=True, exist_ok=True)
    river_dirs = find_river_dirs(f049_out, args.rivers)

    (out / 'F051_args.json').write_text(json.dumps({
        **vars(args),
        'resolved_river_dirs': [str(d) for d in river_dirs],
        'training_objective': 'normalized objective',
    }, indent=2))

    print('[F051] Build GT/error VRTs plus legacy tile-footprint and exact unique-pixel metrics')
    print(f'[F051] f049_out_dir={f049_out}')
    print(f'[F051] output_dir={out}')
    print(f'[F051] rivers={[d.name for d in river_dirs]}')

    summaries = []
    for d in river_dirs:
        summaries.append(process_river(d, out, args))

    (out / 'F051_all_rivers_summary.json').write_text(json.dumps(summaries, indent=2))
    print('[DONE]', out)
