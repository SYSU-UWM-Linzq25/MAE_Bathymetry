#!/usr/bin/env python3
# F049_relax_fullriver_predict_NormalizedObjective_TileAvgVRT_CoreFinalLossOnly.py
# D001c/E001c AnyVisiblePatch RELAX branch.
# Sparse overlap averaging is preserved; output is restricted to true model prediction patches.
"""F049 full-river inference using the D001c AnyVisiblePatch Normalized-objective checkpoint and E001c full-river tiles.
...
3. Keep ONLY pixels in Core_Loss_Mask_Pixel that are also in the model-returned prediction patch mask.
...
Important
---------
Hidden_Mask controls model visibility. Pixels in patches opened as visible by
the E001c AnyVisiblePatch rule are excluded from the final reconstruction.
Final output footprint is:
    Core_Loss_Mask_Pixel AND model prediction_patch_mask AND valid prediction.
"""
...
class SparseTileRecord:
    ...
    loss_path: str
    core_loss_path: str
    core_loss_valid_pixel_count: int
    prediction_patch_pixel_count: int
    final_prediction_pixel_count: int
    excluded_visible_core_loss_pixel_count: int
...
def prepare_sample(item: TileItem, args) -> Dict[str, Any]:
    ...
    # Base pixel footprint only. The final scientific output mask is completed
    # after model inference by intersecting this with the model-returned
    # prediction_patch_mask. This prevents E001c newly-visible patches from
    # entering the reconstructed output or downstream error metrics.
    core_loss_valid_mask = core_loss & valid
    ...
    return {
        ...
        "core_loss": core_loss,
        "core_loss_valid_mask": core_loss_valid_mask,
        "meta": meta,
        "tile_mean_m": mean_m,
        "tile_std_m": std_m,
...
def infer_river(model, items: Sequence[TileItem], river: str, args, device: torch.device) -> Dict[str, Any]:
    ...
    # QA counters are evaluated before overlap averaging.
    total_core_loss_valid_pixels_before_prediction_filter = 0
    total_prediction_patch_pixels = 0
    total_final_prediction_pixels_before_averaging = 0
    total_excluded_visible_core_loss_pixels = 0
    ...
    for start in range(0, n, batch_size):
        ...
        with torch.no_grad():
            ...
            # Expand the model-returned patch masks to the original pixel grid.
            # prediction_patch_mask is the authoritative indicator that a patch
            # was hidden from the encoder and therefore genuinely predicted.
            p = int(model.patch_embed.patch_size[0])
            prediction_patch_px_batch = model.unpatchify(
                prediction_patch_mask.float().unsqueeze(-1).repeat(
                    1, 1, p * p * int(args.in_chans)
                )
            ).detach().float().cpu().numpy()[:, 0] > 0.5
            core_loss_patch_px_batch = model.unpatchify(
                core_loss_patch_mask.float().unsqueeze(-1).repeat(
                    1, 1, p * p * int(args.in_chans)
                )
            ).detach().float().cpu().numpy()[:, 0] > 0.5
        ...
        for j, rec in enumerate(batch):
            ...
            base_core_loss_valid = rec["core_loss_valid_mask"].astype(bool)
            prediction_patch_px = prediction_patch_px_batch[j].astype(bool)
            core_loss_patch_px = core_loss_patch_px_batch[j].astype(bool)

            # Scientific correction:
            # - Core_Loss_Mask_Pixel controls the exact evaluation pixels.
            # - prediction_patch_mask removes patches made visible by E001c.
            # - core_loss_patch_mask is checked as an auxiliary consistency guard.
            mask = (
                base_core_loss_valid
                & prediction_patch_px
                & core_loss_patch_px
                & np.isfinite(pred_m)
            )
            excluded_visible_core_loss = (
                base_core_loss_valid & (~prediction_patch_px)
            )

            core_loss_valid_count = int(base_core_loss_valid.sum())
            prediction_patch_pixel_count = int(prediction_patch_px.sum())
            final_prediction_pixel_count = int(mask.sum())
            excluded_visible_core_loss_count = int(
                excluded_visible_core_loss.sum()
            )

            total_core_loss_valid_pixels_before_prediction_filter += (
                core_loss_valid_count
            )
            total_prediction_patch_pixels += prediction_patch_pixel_count
            total_final_prediction_pixels_before_averaging += (
                final_prediction_pixel_count
            )
            total_excluded_visible_core_loss_pixels += (
                excluded_visible_core_loss_count
            )
            ...
            if local_flat.size == 0:
                skipped.append({
                    ...
                    "tile_path": str(item.tile_path),
                    "skip_reason": (
                        "zero_core_loss_pixels_in_model_prediction_patches"
                    ),
                    "core_loss_valid_pixel_count": core_loss_valid_count,
                    "prediction_patch_pixel_count": prediction_patch_pixel_count,
                    "final_prediction_pixel_count": final_prediction_pixel_count,
                    "excluded_visible_core_loss_pixel_count": (
                        excluded_visible_core_loss_count
                    ),
                })
                continue
    ...
    for rec in sparse_records:
        ...
        avg_path = avg_tile_dir / f"F049_avg_pred_m_{rec.key}_core_final_loss.tif"
        cnt_path = count_tile_dir / f"F049_overlap_count_{rec.key}_core_final_loss.tif"
        ...
        manifest_rows.append({
            ...
            "tile_height": rec.h,
            "tile_width": rec.w,
            "core_loss_valid_pixels_before_prediction_filter": (
                rec.core_loss_valid_pixel_count
            ),
            "prediction_patch_pixels": rec.prediction_patch_pixel_count,
            "core_final_loss_pixels": int(rec.local_flat.size),
            "final_prediction_pixels_after_prediction_patch_filter": (
                rec.final_prediction_pixel_count
            ),
            "excluded_visible_core_loss_pixels": (
                rec.excluded_visible_core_loss_pixel_count
            ),
            "prediction_patch_filter_applied": 1,
    ...
    pred_vrt = out_river / f"F049_fullriver_pred_m_{river}_core_final_loss_avg_tiles.vrt"
    count_vrt = out_river / f"F049_fullriver_overlap_count_{river}_core_final_loss_avg_tiles.vrt"
    ...
    write_csv(out_river / "F049_tileavg_prediction_manifest.csv", manifest_rows)
    write_csv(out_river / "F049_skipped_tiles.csv", skipped)
    ...
    summary = {
        ...
        "mask_branch": "E001c AnyVisiblePatch",
        "training_objective": "normalized objective",
        "model_branch": "NormOnly / D001c AnyVisiblePatch",
        "scientific_fix": "prediction_patch_filter_v1",
        "prediction_patch_filter_applied": True,
        "prediction_patch_mask_source": (
            "model return_aux_masks prediction_patch_mask"
        ),
        "mosaic_mask": (
            "Core_Loss_Mask_Pixel AND model_prediction_patch_mask "
            "AND model_core_loss_patch_mask AND valid_prediction"
        ),
        "hidden_mask_role": (
            "model input visibility; newly-visible E001c patches excluded "
            "from final prediction"
        ),
        "core_loss_valid_pixels_before_prediction_filter": int(
            total_core_loss_valid_pixels_before_prediction_filter
        ),
        "prediction_patch_pixels_before_overlap": int(
            total_prediction_patch_pixels
        ),
        "final_prediction_pixels_before_overlap_averaging": int(
            total_final_prediction_pixels_before_averaging
        ),
        "excluded_visible_core_loss_pixels_before_overlap": int(
            total_excluded_visible_core_loss_pixels
        ),
    ...
    (out_river / "F049_summary.json").write_text(json.dumps(summary, indent=2))
...
def main() -> None:
    ...
    ap.add_argument("--tile_root", default="/tank/data/SFS/xinyis/data/bathymetry/Processed_Results/Tiles_for_MAE_FullRiver_E001c_AnyVisiblePatch/Tiles_1m")
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "F049_summary.json and both VRT outputs are complete; otherwise "
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="Fail unless tile_root identifies the E001c AnyVisiblePatch full-river branch.",
    )
    ...
    if not TIFFFILE_AVAILABLE:
        raise RuntimeError("tifffile is required for this F049 tile-average VRT workflow.")

    if args.require_e001c_anyvisiblepatch_root:
        root_text = str(Path(args.tile_root))
        if "E001c_AnyVisiblePatch" not in root_text:
            raise RuntimeError(
                "Expected E001c AnyVisiblePatch full-river tile root; got: "
                + root_text
            )
    ...
    args_file = out / "F049_args.json"
    if not args_file.exists():
        args_file.write_text(json.dumps(args_payload, indent=2))
    (out / "F049_args_last_invocation.json").write_text(json.dumps(args_payload, indent=2))

    print("[IO] TileAvgVRT=True dense_mosaic_allocated=False tifffile_only=True")
    print(
        "[MASK] corrected footprint = Core_Loss_Mask_Pixel AND "
        "model prediction_patch_mask; E001c newly-visible patches are excluded"
    )
    ...
    def completed_summary(summary_path: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return None
        if summary.get("prediction_patch_filter_applied") is not True:
            print(
                "[RESUME] Old leakage-prone F049 summary detected; "
                f"prediction-patch filter marker is missing: {summary_path.parent}"
            )
            return None
    ...
    for river in rivers:
        river_out = out / river
        summary_path = river_out / "F049_summary.json"
    ...
    for summary_path in sorted(out.glob("*/F049_summary.json")):
        summary = completed_summary(summary_path)
        if summary is not None:
            all_completed.append(summary)
    (out / "F049_all_rivers_summary.json").write_text(json.dumps(all_completed, indent=2))
